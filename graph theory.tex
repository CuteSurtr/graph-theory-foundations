...
% Title settings
\title{\Huge\bfseries Introduction to Graph Theory \\[1em]
...
\begin{enumerate}
    \item $G$ is $d$-degenerate.
    \item Every induced subgraph of $G$ has a vertex of degree at most $d$.
    \item There exists an ordering $v_1, v_2, \ldots, v_n$ of the vertices of $G$ such that each vertex $v_i$ has at most $d$ neighbors among vertices $v_1, v_2, \ldots, v_{i-1}$.
\end{enumerate}
\end{theorem}

\begin{proof}
$(1 \Rightarrow 2)$: This follows directly from the definition of $d$-degeneracy.

$(2 \Rightarrow 3)$: We can construct the required ordering as follows:
\begin{itemize}
\item Start with the original graph $G$.
\item Find a vertex $v_1$ with degree at most $d$ (which exists by assumption).
\item Remove $v_1$ from $G$ to get $G'$.
\item Find a vertex $v_2$ with degree at most $d$ in $G'$ (which exists since $G'$ is an induced subgraph of $G$).
\item Continue this process until all vertices are ordered.
\end{itemize}

In the resulting ordering $v_1, v_2, \ldots, v_n$, each vertex $v_i$ has at most $d$ neighbors among $v_{i+1}, \ldots, v_n$ (since when we selected $v_i$, it had degree at most $d$ in the remaining graph).

$(3 \Rightarrow 1)$: Let $v_1, v_2, \ldots, v_n$ be an ordering where each $v_i$ has at most $d$ neighbors among $v_{i+1}, \ldots, v_n$. For any induced subgraph $H$ of $G$, consider the vertex in $H$ that appears first in this ordering. This vertex has at most $d$ neighbors in $H$, proving that $H$ has a vertex of degree at most $d$.
\end{proof}
...
A $k$-regular graph is $d$-degenerate.
\end{lemma}

\begin{proof}
In a $k$-regular graph, every vertex has degree exactly $k$. When we remove any vertex, the degree of its neighbors decreases by 1, so the minimum degree in the resulting graph is at most $k-1$. Continuing this process, we can always find a vertex of degree at most $k$ in any subgraph, making the graph $d$-degenerate.
\end{proof}
...
\subsection{Algorithms for $d$-Degenerate Graphs}

\begin{algorithm}
\caption{Recognition of $d$-Degenerate Graphs}
...
\begin{algorithmic}[1]
    \Procedure{Is$k$Degenerate}{$G = (V, E)$, $k$}
        ...
        \While{$V(G') \neq \emptyset$}
            \State Find vertex $v \in V(G')$ with minimum degree
            \If{$\deg_{G'}(v) > d$}
                \State \Return \textbf{false} \Comment{Found a subgraph with minimum degree $> d$}
            \EndIf
            \State Remove $v$ from $G'$
        \EndWhile
        \State \Return \textbf{true} \Comment{Every subgraph had a vertex with degree $\leq d$}
    \EndProcedure
\end{algorithmic}
\end{algorithm}

\begin{theorem}[Linear-time Recognition of $d$-degenerate Graphs]
For a fixed value of $k$, determining whether a graph $G$ is $d$-degenerate can be done in linear time $O(|V| + |E|)$.
\end{theorem}

\begin{lemma}[Subgraph Enumeration in $d$-Degenerate Graphs]
For a fixed pattern graph $H$ with $h$ vertices, the number of occurrences of $H$ as a subgraph in a $d$-degenerate graph $G$ with $n$ vertices is at most $O(n \cdot d^{h-1})$, and all such occurrences can be enumerated in time $O(n \cdot d^{h-1})$.
\end{lemma}

\begin{algorithm}
\caption{Enumeration of Maximal Cliques in $d$-Degenerate Graphs}
...
\begin{theorem}[Bron-Kerbosch Algorithm for $d$-Degenerate Graphs]
...
This document explores these connections, with a particular focus on how the concept of graph degeneracy relates to scheduling problems. We will see that $d$-degenerate graphs provide an important structural characterization that can be exploited in various scheduling contexts.
...
\subsection*{Definition and Properties of $d$-degenerate Graphs}
...
\begin{theorem}
For a scheduling problem where jobs correspond to vertices in a conflict graph $G$, if $G$ is $d$-degenerate, then the jobs can be scheduled using at most $k+1$ resources or time slots.
\end{theorem}
This follows directly from the fact that $d$-degenerate graphs are $(k+1)$-colorable, and a proper coloring corresponds to a valid schedule where jobs of the same color can be processed simultaneously.
...
If a precedence graph is $d$-degenerate, then:
\begin{itemize}
...
\begin{theorem}
In a wireless network where nodes correspond to transmitters and edges represent interference between transmitters, if the interference graph is $d$-degenerate, then the transmitters can be scheduled in at most $k+1$ time slots without interference.
\end{theorem}
...
Degeneracy is also useful in distributed computing environments. In a distributed system where tasks are assigned to processors and the task dependency graph is $d$-degenerate, a valid task allocation can be found using at most $k+1$ communication rounds.

\begin{algorithm}
\caption{Distributed Task Scheduling for $d$-degenerate Dependency Graphs}
\label{alg:distributed-scheduling}
\begin{algorithmic}[1]
    \Require Dependency graph $G = (V, E)$ with degeneracy $d$
...
\begin{enumerate}
    ...
    \item \textbf{Adaptability}: The approach works for any $d$-degenerate graph, regardless of other structural properties
...
Consider a software development project with tasks represented as vertices and dependencies as edges. The dependency graph is typically sparse and often $d$-degenerate for small $k$ due to modular design practices. Using the degeneracy ordering:
...
Suppose we have students $s_1, s_2, \ldots, s_m$, each has to write a set $E_1, E_2, \ldots, E_m$ of exams.

What is the minimum number of time slots to schedule the exams so that no student has to write two exams at the same time?
...
and
\begin{align*}
s_1 &\text{ has to write exams } 1 \text{ and } 4 \\
s_2 &\text{ has to write exams } 1, 2 \text{ and } 5 \\
s_3 &\text{ has } \cdots \cdots \cdots \cdots \cdots 2, 3, 4 \\
s_4 &\text{ has } \cdots \cdots \cdots \cdots \cdots 3, 5
\end{align*}

Form a graph $G$ where $V(G)$ is the set of exams and $\{i,j\}$ is an edge if some student has to write both exams $i$ and $j$.

In the example,

\begin{center}
\begin{tikzpicture}[scale=1.5]
% Define vertices in a pentagonal arrangement
\node[circle,draw,fill=green!30] (1) at (90:1.5) {1};
\node[circle,draw,fill=purple!30] (2) at (162:1.5) {2};
\node[circle,draw,fill=green!30] (3) at (234:1.5) {3};
\node[circle,draw,fill=blue!30] (4) at (306:1.5) {4};
\node[circle,draw,fill=purple!30] (5) at (18:1.5) {5};

% Draw edges based on your specification
% Vertex 1 connects to: 2, 4, 5
\draw (1) -- (2);
\draw (1) -- (4);
\draw (1) -- (5);

% Vertex 2 connects to: 1, 3, 4, 5
\draw (2) -- (3);
\draw (2) -- (4);
\draw (2) -- (5);

% Vertex 3 connects to: 2, 4, 5
\draw (3) -- (4);
\draw (3) -- (5);
...
% Vertex 5 connects to: 1, 2, 3
% (already drawn above)

% NO edge between 1-3 and NO edge between 4-5

% Label
\node at (-2.5,1) {$G:$};

% Annotations
\node[right] at (2.5,0) {\parbox{3cm}{This is the\\schedule\\graph.$\chi(G) = 3$\\Since not bipartite.}};
\end{tikzpicture}
\end{center}

\small Figure 148: {\parbox{0.8\textwidth}{The minimum number of time slots equals the chromatic number of the schedule graph.}}

\vspace{0.5cm}

3 time slots $\begin{cases}
\text{Exam 2 at 9am} \\
\text{Exams 1 and 3 at 10am} \\
\text{Exams 4 and 5 at 11am}
\end{cases}$
...
\subsection{Vertex and Edge-Coloring Exercises}

\subsubsection{Easy Questions}
\begin{enumerate}
\item Define the chromatic number $\chi(G)$ of a graph $G$. What is the chromatic number of a complete graph $K_n$?

\item State König's theorem for bipartite graphs. What does it say about the relationship between vertex cover and maximum matching?

\item What is the edge chromatic number (chromatic index) $\chi'(G)$ of a graph $G$? How does it relate to the maximum degree $\Delta(G)$?

\item Find the chromatic number of the cycle $C_5$. Explain your reasoning.

\item Prove that any bipartite graph has chromatic number at most 2.

\item What is the chromatic number of the complete bipartite graph $K_{m,n}$ where $m, n \geq 1$?

\item State Vizing's theorem. What are the two possible values for the chromatic index of any simple graph?

\item Find the chromatic index of the complete graph $K_4$. Show a proper edge coloring.

\item Explain why the chromatic number of any graph $G$ satisfies $\chi(G) \geq \omega(G)$, where $\omega(G)$ is the clique number.

\item What is a degenerate graph? Give an example of a 2-degenerate graph.
\end{enumerate}

\subsubsection{Medium Questions}
\begin{enumerate}[start=11]
\item Prove that if $G$ is a graph with maximum degree $\Delta$, then $\chi(G) \leq \Delta + 1$.

\item Show that the chromatic number of the Petersen graph is 3 by providing an explicit 3-coloring.

\item Prove König's theorem: In a bipartite graph, the size of a maximum matching equals the size of a minimum vertex cover.

\item Let $G$ be a graph with chromatic number $k$. Prove that $G$ contains a subgraph with minimum degree at least $k-1$.

\item Find the chromatic index of the complete bipartite graph $K_{3,4}$. Is this graph Class 1 or Class 2 according to Vizing's classification?

\item Prove that if $G$ is a bipartite graph with maximum degree $\Delta$, then $\chi'(G) = \Delta$.

\item Show that the chromatic number of the wheel graph $W_n$ (a cycle $C_n$ with an additional central vertex connected to all vertices of the cycle) depends on whether $n$ is odd or even.

\item Prove that any planar graph has chromatic number at most 4 (Four Color Theorem - you may state this without proof, but explain its significance).

\item Let $G$ be a $k$-degenerate graph. Prove that $\chi(G) \leq k + 1$ using the degeneracy ordering.

\item Describe the greedy coloring algorithm and analyze its performance. Under what conditions does it achieve the optimal coloring?
\end{enumerate}

\subsubsection{Hard Questions}
\begin{enumerate}[start=21]
\item Prove or disprove: If $G$ is a graph where every subgraph has a vertex of degree at most $k$, then $\chi(G) \leq k + 1$.

\item Let $G$ be a graph with $n$ vertices and chromatic number $\chi(G) = k$. Prove that $G$ has at least $\binom{k}{2}$ edges.

\item Prove Vizing's theorem: For any simple graph $G$ with maximum degree $\Delta$, either $\chi'(G) = \Delta$ or $\chi'(G) = \Delta + 1$.

\item Show that determining whether a graph is 3-colorable is NP-complete, while determining whether a graph is 2-colorable is polynomial-time solvable.

\item Let $G$ be a graph with girth $g$ (length of shortest cycle) and chromatic number $k$. Prove that $G$ has at least $1 + \Delta + \Delta(\Delta-1) + \cdots + \Delta(\Delta-1)^{\lfloor g/2 \rfloor - 1}$ vertices.

\item Prove Shannon's theorem: For any graph $G$, $\chi'(G) \leq \lfloor \frac{3\Delta}{2} \rfloor$ where $\Delta$ is the maximum degree.

\item Let $G$ be a graph such that every subgraph of $G$ has degeneracy at most $k$. Prove that $\chi(G) \leq k + 1$ and that this bound is tight.

\item Design a polynomial-time algorithm to find the chromatic index of a bipartite graph. Analyze its time complexity.

\item Prove that if $G$ is a graph with $\chi(G) = k$ and $\alpha(G) = \ell$ (independence number), then $n \geq k \cdot \ell$.

\item Investigate the relationship between the chromatic number and the fractional chromatic number. Show that they can differ by an arbitrarily large factor.
\end{enumerate}

\subsubsection{Diagram Questions}
\begin{enumerate}[start=31]
\item Draw the complete graph $K_5$ and provide an optimal vertex coloring. Explain why this is the minimum number of colors needed.

\item Illustrate König's theorem by drawing a bipartite graph, finding a maximum matching, and showing the corresponding minimum vertex cover.

\item Draw the Petersen graph and show that it requires exactly 3 colors. Mark the color classes clearly.

\item Create a graph that is Class 2 according to Vizing's classification (i.e., $\chi'(G) = \Delta(G) + 1$). Show why $\Delta(G)$ colors are insufficient.

\item Draw the wheel graph $W_5$ and $W_6$. Show optimal colorings for both and explain why their chromatic numbers differ.

\item Illustrate the greedy coloring algorithm on a specific graph where it does NOT produce an optimal coloring. Show both the greedy result and the optimal coloring.

\item Draw a 3-degenerate graph with 8 vertices. Show the degeneracy ordering and use it to find a 4-coloring.

\item Create a bipartite graph and show its edge coloring using exactly $\Delta$ colors, illustrating the result that bipartite graphs are always Class 1.

\item Draw two different graphs, both with maximum degree 3, where one is Class 1 and the other is Class 2. Show the edge colorings for both.

\item Illustrate Shannon's bound by drawing a graph where the chromatic index is close to $\frac{3\Delta}{2}$. Show the edge coloring that achieves this bound.
\end{enumerate}
...
\begin{definition}
A graph is \textit{planar} if it can be drawn in the plane without crossing edges.
\end{definition}

\begin{example}
The complete graph $K_4$ is planar. Although it may initially appear to have crossing edges when drawn as a square with diagonals, it can be redrawn without crossings.

\begin{figure}[ht]
\centering
\begin{tikzpicture}[
    scale=1.0,
    vertex/.style={circle, draw, fill=white, minimum size=0.6cm, inner sep=0pt},
    thick_edge/.style={line width=1.2pt}
]
    % Non-planar looking K4
    \begin{scope}[xshift=-4cm]
        \node[vertex] (A) at (0,2) {$v_1$};
        \node[vertex] (B) at (2,2) {$v_2$};
        \node[vertex] (C) at (2,0) {$v_3$};
        \node[vertex] (D) at (0,0) {$v_4$};

        \draw[thick_edge, color=blue] (A) -- (B) -- (C) -- (D) -- (A);
        \draw[thick_edge, color=red] (A) -- (C);
        \draw[thick_edge, color=red] (B) -- (D);

        \node at (1,-0.8) {\small Appears to cross};
    \end{scope}

    % Arrow
    \draw[->, thick, color=gray] (-1.5,1) -- (-0.5,1);

    % Planar K4
    \begin{scope}[xshift=2cm]
        \node[vertex] (A) at (0,1.5) {$v_1$};
        \node[vertex] (B) at (1.5,2) {$v_2$};
        \node[vertex] (C) at (2,0.5) {$v_3$};
        \node[vertex] (D) at (0.5,0) {$v_4$};

        \draw[thick_edge, color=blue] (A) -- (B) -- (C) -- (D) -- (A);
        \draw[thick_edge, color=green] (A) to[bend right=30] (C);
        \draw[thick_edge, color=green] (B) to[bend left=40] (D);

        \node at (1,-0.8) {\small No crossings!};
    \end{scope}
\end{tikzpicture}
\\
\small Figure 149: $K_4$ drawn in non-planar and planar representations
\label{fig:k4_planar}
\end{figure}
...
\subsection{Faces of Planar Graphs}
\begin{definition}
If $G$ is a planar graph, then the \textit{faces} of $G$ are the connected regions of the plane that are disjoint from the drawing of $G$.
\end{definition}

\begin{definition}
The \textit{degree} of a face $f$, denoted $\deg(f)$, is the length of the shortest closed walk containing all points on the boundary of $f$.
\end{definition}

\begin{example}
Consider a triangle with an additional edge:
...
\begin{tikzpicture}[
    ...
    \node[vertex] (D) at (0,0.7) {$d$};

    \draw[thick_edge, color=blue] (A) -- (B) -- (C) -- (A);
    \draw[thick_edge, color=red] (B) -- (D) -- (C);

    % Face degrees
    \node at (-2.2,1.5) {\small $\deg(f_1) = 4$};
    \node at (2.2,1.5) {\small $\deg(f_2) = 4$};
    \node at (0,-1.8) {\small $\deg(f_3) = 4$};
\end{tikzpicture}
\\
\small Figure 150: Triangle with additional edge showing three faces
\label{fig:faces_example}
\end{figure}

Each face has degree 4. Note that edges $bd$ and $dc$ are each counted once for faces $f_1$ and $f_2$, but edges $ab$, $ac$, and $bc$ are shared with the outer face.
\end{example}

\subsection{Fundamental Theorems}
\begin{lemma}[Handshaking Lemma for Faces]
...
\begin{example}
Verification with the triangle example above:
...
\begin{tikzpicture}[
    scale=1.0,
    vertex/.style={circle, draw, fill=white, minimum size=0.6cm, inner sep=0pt},
    thick_edge/.style={line width=1.5pt}
]
    \node[vertex] (A) at (0,2) {$a$};
    \node[vertex] (B) at (-1.5,0) {$b$};
    \node[vertex] (C) at (1.5,0) {$c$};
    ...
    \draw[thick_edge, color=red] (A) -- (B);
    \draw[thick_edge, color=blue] (B) -- (C);
    \draw[thick_edge, color=green] (C) -- (A);
    \draw[thick_edge, color=orange] (B) -- (D);
    \draw[thick_edge, color=purple] (D) -- (C);

    % Edge labels
    \node at (-0.8,1.2) {\small $e_1$};
    \node at (0,-0.2) {\small $e_2$};
    \node at (0.8,1.2) {\small $e_3$};
    \node at (-0.8,0.4) {\small $e_4$};
    \node at (0.8,0.4) {\small $e_5$};

    \node at (0,-1.5) {\small $\sum \deg(f) = 4 + 4 + 4 = 12 = 2 \times 5 = 2|E|$};
\end{tikzpicture}
\\
\small Figure 151: Verification of Handshaking Lemma for Faces
\label{fig:handshaking_faces}
\end{figure}
\end{example}
...
\begin{example}
Verification with our triangle example:
$|V| = 4$, $|E| = 5$, $|F| = 3$, so $4 - 5 + 3 = 2$ (correct)
\end{example}

\subsection{Applications: Proving Non-Planarity}
...
\begin{tikzpicture}[
    scale=1.0,
    vertex/.style={circle, draw, fill=white, minimum size=0.6cm, inner sep=0pt},
    thick_edge/.style={line width=1.0pt}
]
    % K5 vertices in pentagon formation
    \foreach \i in {1,2,3,4,5} {
        \node[vertex] (\i) at ({72*(\i-1)}:2.2) {$v_{\i}$};
    }

    % Draw all edges (this will create crossings)
    \foreach \i in {1,2,3,4,5} {
        \foreach \j in {1,2,3,4,5} {
            \ifnum\i<\j
                \draw[thick_edge, color=blue] (\i) -- (\j);
            \fi
        }
    }

    \node at (0,-3.2) {\small $K_5$ cannot be drawn without crossings};
    \node at (0,-3.7) {\small $|V| = 5$, $|E| = 10$, but if planar: $|E| \leq 3(|V|-2) = 9$};
\end{tikzpicture}
\\
\small Figure 152: $K_5$ showing inevitable edge crossings
\label{fig:k5_nonplanar}
\end{figure}
\end{example}
...
\begin{tikzpicture}[
    scale=1.0,
    vertex/.style={circle, draw, minimum size=0.6cm, inner sep=0pt},
    thick_edge/.style={line width=1.0pt}
]
    % Left vertices
    \foreach \i in {1,2,3} {
        \node[vertex, fill=red!20] (L\i) at (-2.5, {1.5*(\i-2)}) {$u_{\i}$};
    }

    % Right vertices
    \foreach \i in {1,2,3} {
        \node[vertex, fill=blue!20] (R\i) at (2.5, {1.5*(\i-2)}) {$v_{\i}$};
    }

    % Draw all edges between left and right
    \foreach \i in {1,2,3} {
        \foreach \j in {1,2,3} {
            \draw[thick_edge, color=purple] (L\i) -- (R\j);
        }
    }

    \node at (0,-2.8) {\small $K_{3,3}$ cannot be drawn without crossings};
    \node at (0,-3.3) {\small $|V| = 6$, $|E| = 9$, but if planar: $|E| \leq 2(|V|-2) = 8$};
\end{tikzpicture}
\\
\small Figure 153: $K_{3,3}$ complete bipartite graph showing crossings
\label{fig:k33_nonplanar}
\end{figure}
\end{example}

\begin{example}
Planar vs Non-planar comparison:

\begin{figure}[ht]
\centering
\begin{tikzpicture}[
    scale=0.8,
    vertex/.style={circle, draw, fill=white, minimum size=0.5cm, inner sep=0pt},
    thick_edge/.style={line width=1.0pt}
]
    ...
    \begin{scope}[xshift=-4cm]
        ...
        \draw[thick_edge, color=green] (A) -- (B) -- (C) -- (D) -- (E) -- (F) -- (A);
        \draw[thick_edge, color=blue] (A) -- (D);
        \draw[thick_edge, color=blue] (B) -- (E);
        \draw[thick_edge, color=blue] (C) -- (F);

        \node at (0,-2.8) {\small Planar: $|V|=6$, $|E|=9$};
        \node at (0,-3.3) {\small Satisfies: $9 \leq 3(6-2) = 12$ (valid)};
    \end{scope}

    % Non-planar graph example (K5)
    \begin{scope}[xshift=4cm]
        \foreach \i in {1,2,3,4,5} {
            \node[vertex] (\i) at ({72*(\i-1)}:1.5) {\tiny $v_{\i}$};
        }

        \foreach \i in {1,2,3,4,5} {
            \foreach \j in {1,2,3,4,5} {
                \ifnum\i<\j
                    \draw[thick_edge, color=red] (\i) -- (\j);
                \fi
            }
        }

        \node at (0,-2.8) {\small Non-planar: $K_5$};
        \node at (0,-3.3) {\small Violates: $10 > 3(5-2) = 9$ (invalid)};
    \end{scope}
\end{tikzpicture}
\\
\small Figure 154: Comparison of planar and non-planar graphs
\label{fig:planar_comparison}
\end{figure}
\end{example}

\begin{remark}
The results that $K_5$ and $K_{3,3}$ are not planar will be on the exam.
\end{remark}
...
% TikZ libraries
\usetikzlibrary{patterns,shapes.geometric,decorations.pathreplacing}
...
% Colors
\definecolor{guardcolor}{RGB}{220,20,60}
\definecolor{visiblearea}{RGB}{173,216,230}
\definecolor{polygoncolor}{RGB}{70,130,180}

\subsection{Introduction and Formal Definitions of Art Gallery Problems}

\begin{definition}[Simple Polygon]
A \textbf{simple polygon} $P$ is a Jordan curve in $\mathbb{R}^2$ consisting of a finite sequence of line segments $e_1, e_2, \ldots, e_n$ called edges, where:
\begin{itemize}[itemsep=0pt]
\item Each edge $e_i$ connects vertices $v_i$ and $v_{i+1}$ (with $v_{n+1} = v_1$)
\item No two non-consecutive edges intersect
\item The polygon encloses a bounded region
\end{itemize}
\end{definition}

\begin{definition}[Visibility]
Two points $p, q \in P$ are \textbf{visible} to each other if the line segment $\overline{pq}$ lies entirely within $P$. We write $p \sim q$ if $p$ and $q$ are visible to each other.
\end{definition}

\begin{definition}[Guard Set]
A set $G \subseteq P$ is a \textbf{guard set} for polygon $P$ if for every point $p \in P$, there exists a guard $g \in G$ such that $p \sim g$.
\end{definition}

\begin{definition}[The Art Gallery Problem]
Given a simple polygon $P$ with $n$ vertices, determine:
\begin{enumerate}[itemsep=0pt]
\item $g(P)$: The minimum number of guards needed to guard $P$
\item $g(n)$: The maximum of $g(P)$ over all simple polygons with $n$ vertices
\end{enumerate}
\end{definition}


\begin{theorem}[Chv\'{a}tal's Art Gallery Theorem, 1975]
For any simple polygon $P$ with $n$ vertices,
$$g(n) = \left\lfloor \frac{n}{3} \right\rfloor$$
\end{theorem}


\subsection{Construction and Visualization}

The comb polygon demonstrates that the bound $\lfloor n/3 \rfloor$ is tight. It resembles the triangular teeth of Toblerone chocolate.

\begin{center}
\begin{tikzpicture}[scale=0.8]
% Draw the comb polygon
\draw[thick, polygoncolor] 
  (0,0) -- (0.5,2) -- (1,0) -- (2,0) -- (2.5,2) -- (3,0) -- 
  (4,0) -- (4.5,2) -- (5,0) -- (6,0) -- (6.5,2) -- (7,0) -- 
  (8,0) -- (0,0);

% Add vertex labels
\foreach \x/\y/\label in {0.5/2/1, 1/0/2, 2/0/3, 2.5/2/4, 3/0/5, 4/0/6, 4.5/2/7, 5/0/8, 6/0/9, 6.5/2/10, 7/0/11, 8/0/12, 0/0/13}
  \node[circle,fill=white,draw,inner sep=1pt] at (\x,\y) {\tiny\label};

% Add guards
\foreach \x in {1.25, 3.25, 5.25, 6.75}
  \node[circle,fill=guardcolor,inner sep=2pt] at (\x,0.7) {};

% Add title
\node[above] at (4,2.5) {\textbf{Comb Polygon with $n=13$ vertices (requires 4 guards)}};

% Add brace to show one tooth
\draw[decorate,decoration={brace,amplitude=5pt}] (0.5,2.2) -- (2,2.2);
\node[above] at (1.25,2.4) {\small One ``tooth''};
\end{tikzpicture}
\end{center}

\begin{definition}[Comb Polygon]
A \textbf{comb polygon} $C_k$ with $n = 3k+1$ vertices is constructed as:
\begin{itemize}[itemsep=0pt]
\item $k$ triangular ``teeth'' protruding upward
\item A horizontal base connecting all teeth
\item Each tooth requires its own guard due to visibility constraints
\end{itemize}
\end{definition}

\begin{lemma}[Comb Lower Bound]
The comb polygon $C_k$ with $n = 3k+1$ vertices requires exactly $k = \lfloor n/3 \rfloor$ guards.
\end{lemma}

\begin{proof}
Each triangular tooth creates a visibility barrier. The peak of each tooth is only visible from within that tooth. Thus, each tooth requires its own guard, giving $k = \lfloor n/3 \rfloor$ guards.
\end{proof}
...
A \textbf{triangulation} of a simple polygon $P$ is a maximal set of non-crossing diagonals that partition $P$ into triangles.
\end{definition}

\begin{theorem}[Triangulation Existence]
Every simple polygon with $n \geq 3$ vertices can be triangulated into exactly $n-2$ triangles using $n-3$ diagonals.
\end{theorem}
...
\draw[thick, polygoncolor] (0,0) -- (2,0) -- (2,1.5) -- (1,2) -- (0,1.5) -- cycle;
\foreach \x/\y/\label in {0/0/1, 2/0/2, 2/1.5/3, 1/2/4, 0/1.5/5}
  \node[circle,fill=white,draw,inner sep=1pt] at (\x,\y) {\tiny\label};
\node[below] at (1,-0.5) {\textbf{Original Pentagon}};
\end{scope}
...
\begin{scope}[xshift=3cm]
\draw[thick, polygoncolor] (0,0) -- (2,0) -- (2,1.5) -- (1,2) -- (0,1.5) -- cycle;
\draw[dashed, red] (0,0) -- (2,1.5);
\draw[dashed, red] (0,0) -- (1,2);
\foreach \x/\y/\label in {0/0/1, 2/0/2, 2/1.5/3, 1/2/4, 0/1.5/5}
  \node[circle,fill=white,draw,inner sep=1pt] at (\x,\y) {\tiny\label};
\node[below] at (1,-0.5) {\textbf{After Triangulation}};
\node[red,right] at (2.2,0.7) {\small 3 triangles};
\node[red,right] at (2.2,0.4) {\small 2 diagonals};
\end{scope}
\end{tikzpicture}
\end{center}
...
\begin{algorithmic}[1]
\Require Simple polygon $P$ with vertices $v_1, \ldots, v_n$
\Ensure Triangulation of $P$
\While{$n > 3$}
    \State Find an ear vertex $v_i$ (convex vertex with no vertices inside triangle $v_{i-1}v_iv_{i+1}$)
    \State Add diagonal $v_{i-1}v_{i+1}$
    \State Remove vertex $v_i$
    \State $n \leftarrow n-1$
\EndWhile
\State \textbf{Complexity:} $O(n^2)$ time, $O(n)$ space
\end{algorithmic}
\end{algorithm}

\subsection{Three-Coloring}

\begin{theorem}[3-Colorability of Triangulated Polygons]
Every triangulated simple polygon has a proper 3-coloring of its vertices such that no triangle has vertices of the same color.
\end{theorem}

\begin{center}
\begin{tikzpicture}[scale=1.5]
% Draw triangulated polygon with 3-coloring
\coordinate (A) at (0,0);
\coordinate (B) at (2,0);
...
% Draw polygon
\draw[thick, polygoncolor] (A) -- (B) -- (C) -- (D) -- (E) -- (F) -- cycle;
...
% Color vertices
\node[circle,fill=red,draw,minimum size=8pt] at (A) {};
\node[circle,fill=blue,draw,minimum size=8pt] at (B) {};
\node[circle,fill=green!70!black,draw,minimum size=8pt] at (C) {};
\node[circle,fill=blue,draw,minimum size=8pt] at (D) {};
\node[circle,fill=green!70!black,draw,minimum size=8pt] at (E) {};
\node[circle,fill=red,draw,minimum size=8pt] at (F) {};

% Add labels
\node[below left] at (A) {R};
\node[below right] at (B) {B};
\node[right] at (C) {G};
\node[above] at (D) {B};
\node[above left] at (E) {G};
\node[left] at (F) {R};

\node[below] at (1,-0.7) {\textbf{3-Coloring Example}};
\node[below] at (1,-1) {Each triangle has exactly one vertex of each color};
\end{tikzpicture}
\end{center}

\begin{proof}[Proof of 3-Colorability]
By induction on the number of triangles:
\begin{itemize}[itemsep=0pt]
\item \textbf{Base case:} A single triangle can be 3-colored
\item \textbf{Inductive step:} Remove an ear, 3-color the remaining polygon, then color the removed vertex with the third color not used by its two neighbors
\end{itemize}
\end{proof}

\subsection{The Complete Guard Placement Algorithm}

\begin{algorithm}
\caption{Art Gallery Guard Placement}
\begin{algorithmic}[1]
\Require Simple polygon $P$ with $n$ vertices
\Ensure Guard set $G$ with $|G| \leq \lfloor n/3 \rfloor$
\State Triangulate $P$ into $n-2$ triangles
\State 3-color the vertices: $V = R \cup B \cup G$ (Red, Blue, Green)
\State $G \leftarrow \arg\min\{|R|, |B|, |G|\}$
\State \Return Guard set $G$
\end{algorithmic}
\end{algorithm}

\begin{theorem}[Correctness of Algorithm]
The algorithm produces a valid guard set with at most $\lfloor n/3 \rfloor$ guards.
\end{theorem}

\begin{proof}
Since we have a 3-coloring with color classes $R$, $B$, $G$, we have $|R| + |B| + |G| = n$. By the pigeonhole principle, $\min\{|R|, |B|, |G|\} \leq n/3$. The chosen color class guards all triangles since each triangle contains exactly one vertex of each color.
\end{proof}
...
\begin{tikzpicture}[scale=1.0]
% Draw a polygon
\draw[thick, polygoncolor] (0,0) -- (3,0) -- (4,1) -- (3,3) -- (1,2.5) -- (0,1) -- cycle;

% Guard position
\coordinate (G) at (1.5,1.2);
\node[circle,fill=guardcolor,inner sep=3pt,label=above:Guard] at (G) {};

% Visibility region (simplified)
\fill[visiblearea,opacity=0.3] (0,0) -- (1.5,1.2) -- (3,0) -- (4,1) -- (1.5,1.2) -- (0,1) -- cycle;

% Visibility lines
\draw[dashed,guardcolor] (G) -- (0,0);
\draw[dashed,guardcolor] (G) -- (3,0);
\draw[dashed,guardcolor] (G) -- (4,1);
\draw[dashed,guardcolor] (G) -- (0,1);

% Blocked region
\fill[pattern=north east lines,pattern color=red,opacity=0.5] (1.5,1.2) -- (3,3) -- (1,2.5) -- cycle;

\node[below] at (2,-0.5) {\textbf{Visibility Region from a Guard}};
\node[red,right] at (4.2,2) {\small Blocked};
\node[red,right] at (4.2,1.7) {\small region};
\node[cyan!70!blue,right] at (4.2,0.5) {\small Visible};
\node[cyan!70!blue,right] at (4.2,0.2) {\small region};
\end{tikzpicture}
\end{center}
...
\begin{theorem}[Kahn-Klawe-Kleitman, 1983]
For orthogonal polygons (edges parallel to coordinate axes) with $n$ vertices:
$$g_{\text{orth}}(n) = \left\lfloor \frac{n}{4} \right\rfloor$$
\end{theorem}
...
\draw[thick, polygoncolor] 
  (0,0) -- (4,0) -- (4,1) -- (3,1) -- (3,2) -- (4,2) -- (4,3) -- 
  (0,3) -- (0,2) -- (1,2) -- (1,1) -- (0,1) -- cycle;

% Place guards
\node[circle,fill=guardcolor,inner sep=2pt] at (2,0.5) {};
\node[circle,fill=guardcolor,inner sep=2pt] at (2,2.5) {};

\node[below] at (2,-0.5) {\textbf{Orthogonal Polygon (12 vertices, 2 guards)}};
\end{tikzpicture}
\end{center}

\subsection{Mobile Guards}

\begin{theorem}[O'Rourke, 1983]
For mobile guards that can patrol along edges:
$$g_{\text{mobile}}(n) = \left\lfloor \frac{n}{4} \right\rfloor$$
\end{theorem}

\subsection{Monotone Polygons}

\begin{definition}[Monotone Polygon]
A polygon is \textbf{$y$-monotone} if any horizontal line intersects it in at most two points.
\end{definition}

\begin{theorem}
For $y$-monotone polygons with $n$ vertices:
$$g_{\text{monotone}}(n) = \left\lfloor \frac{n-2}{2} \right\rfloor$$
\end{theorem}


\begin{definition}[Art Gallery Decision Problem]
\textsc{Input:} Simple polygon $P$ and integer $k$\\
\textsc{Question:} Is $g(P) \leq k$?
\end{definition}

\begin{theorem}[Lee and Lin, 1986]
The Art Gallery Decision Problem is NP-complete, even for simple polygons.
\end{theorem}

\begin{theorem}[Approximation Algorithm, Ghosh 1987]
There exists an $O(\log n)$-approximation algorithm for the Art Gallery Problem using a greedy set cover approach.
\end{theorem}


\begin{definition}[Star-Shaped Polygon]
A polygon $P$ is \textbf{star-shaped} if there exists a point $p \in P$ such that $p$ can see all of $P$.
\end{definition}

\begin{definition}[Kernel]
The \textbf{kernel} of polygon $P$ is:
$$\text{Kernel}(P) = \{p \in P : \forall q \in P, p \sim q\}$$
\end{definition}

\begin{center}
\begin{tikzpicture}[scale=1.0]
% Star-shaped polygon
\draw[thick, polygoncolor] (2,1) -- (3,0) -- (4,0.5) -- (3.5,2) -- (2.5,2.5) -- (1,2) -- (0.5,1) -- cycle;

% Kernel region
\fill[pattern=dots,pattern color=green!70!black,opacity=0.7] (1.8,1.2) -- (2.2,1.2) -- (2.2,1.8) -- (1.8,1.8) -- cycle;

% Point in kernel
\node[circle,fill=green!70!black,inner sep=2pt,label=above:Kernel] at (2,1.5) {};

% Visibility lines to all vertices
\foreach \point in {(2,1), (3,0), (4,0.5), (3.5,2), (2.5,2.5), (1,2), (0.5,1)}
  \draw[dashed,green!70!black,opacity=0.7] (2,1.5) -- \point;

\node[below] at (2,-0.5) {\textbf{Star-Shaped Polygon with Kernel}};
\end{tikzpicture}
\end{center}

\begin{theorem}
A polygon is star-shaped if and only if its kernel is non-empty. Star-shaped polygons require only 1 guard.
\end{theorem}


\subsection{Fortress Problem}
Guard the \textbf{exterior} of a polygon from guards placed outside.
$$g_{\text{fortress}}(n) = \left\lceil \frac{n}{2} \right\rceil$$

\subsection{Prison Yard Problem}
Guards inside must see both the interior and the boundary.
$$g_{\text{prison}}(n) = \left\lceil \frac{n}{3} \right\rceil$$


\begin{theorem}[Negative Result for 3D]
There exist polyhedra with $n$ vertices that require $\Omega(n)$ guards, showing no constant fraction suffices in 3D.
\end{theorem}

This demonstrates that the art gallery problem becomes much harder in higher dimensions.


\begin{center}
\begin{tabular}{|l|c|l|}
\hline
\textbf{Polygon Type} & \textbf{Guard Bound} & \textbf{Reference} \\
\hline
Simple polygon & $\lfloor n/3 \rfloor$ & Chv\'{a}tal (1975) \\
Orthogonal polygon & $\lfloor n/4 \rfloor$ & Kahn-Klawe-Kleitman (1983) \\
Mobile guards & $\lfloor n/4 \rfloor$ & O'Rourke (1983) \\
$y$-monotone polygon & $\lfloor (n-2)/2 \rfloor$ & \\
Fortress problem & $\lceil n/2 \rceil$ & \\
Star-shaped polygon & $1$ & \\
\hline
\end{tabular}
\end{center}
...
\begin{problem}
Prove that a convex polygon with $n$ vertices requires only 1 guard.
\end{problem}

\begin{problem}
Show that the visibility graph of a convex polygon is the complete graph $K_n$.
\end{problem}

\begin{problem}
Construct a polygon with 15 vertices that requires exactly 5 guards.
\end{problem}

\begin{problem}
Prove that every quadrilateral requires at most 1 guard.
\end{problem}

\subsection{Open Problems}

\begin{enumerate}[itemsep=0pt]
\item Better approximation algorithms (current best: $O(\log n)$)
\item Exact algorithms for special polygon classes
\item Dynamic art gallery problems with moving targets
\item Efficient algorithms for 3D polyhedron guarding
\end{enumerate}

\newpage

\subsection{Planar Graphs and Art Gallery Problems}

\subsubsection{Easy Questions}
\begin{enumerate}
\item Define a planar graph. What is the difference between a planar graph and a plane graph?

\item State Euler's formula for connected planar graphs. What does each variable represent?

\item What is the degree of a face in a planar graph? How is it calculated?

\item Prove the handshaking lemma for faces: For any planar graph $G$, $\sum_{f \in F(G)} \deg(f) = 2|E(G)|$.

\item Find the number of faces in a connected planar graph with 8 vertices and 12 edges using Euler's formula.

\item Show that the complete graph $K_4$ is planar by drawing it without edge crossings.

\item Define the chromatic number $\chi(G)$ of a graph $G$. What is $\chi(K_4)$?

\item What is a guard set in the context of the art gallery problem? Define visibility between two points.

\item State Chvátal's Art Gallery Theorem. What does it tell us about the number of guards needed?

\item What is a star-shaped polygon? How many guards does it require and why?
\end{enumerate}

\subsubsection{Medium Questions}
\begin{enumerate}[start=11]
\item Prove that $K_5$ is not planar using Euler's formula and the fact that every face in a simple planar graph has degree at least 3.

\item Show that the complete bipartite graph $K_{3,3}$ is not planar by counting faces and applying appropriate bounds.

\item For a simple planar graph with $n \geq 3$ vertices, prove that $|E| \leq 3|V| - 6$.

\item Construct a comb polygon with 13 vertices and show that it requires exactly 4 guards, demonstrating that the bound $\lfloor n/3 \rfloor$ is tight.

\item Prove that every simple polygon with $n$ vertices can be triangulated into exactly $n-2$ triangles using $n-3$ diagonals.

\item Show that every triangulated simple polygon has a proper 3-coloring of its vertices such that no triangle has vertices of the same color.

\item Describe the algorithm for solving the art gallery problem using triangulation and 3-coloring. Prove its correctness.

\item For triangle-free planar graphs, prove the stronger bound $|E| \leq 2|V| - 4$.

\item What is the chromatic number of a planar graph? State the Four Color Theorem and explain its significance for art gallery problems.

\item Define the kernel of a polygon. Prove that a polygon is star-shaped if and only if its kernel is non-empty.
\end{enumerate}

\subsubsection{Hard Questions}
\begin{enumerate}[start=21]
\item Prove Kuratowski's theorem (statement only): A graph is planar if and only if it contains no subdivision of $K_5$ or $K_{3,3}$ as a subgraph.

\item Let $G$ be a planar graph with $n$ vertices where every face has degree at least $k$. Derive a bound on the number of edges in terms of $n$ and $k$.

\item Prove that the art gallery decision problem (determining if $k$ guards suffice for a given polygon) is NP-complete.

\item For orthogonal polygons (edges parallel to coordinate axes), state and prove the bound $g_{\text{orth}}(n) = \lfloor n/4 \rfloor$.

\item Show that for $y$-monotone polygons with $n$ vertices, $g_{\text{monotone}}(n) = \lfloor (n-2)/2 \rfloor$.

\item Prove that in 3D, there exist polyhedra with $n$ vertices that require $\Omega(n)$ guards, showing that no constant fraction suffices.

\item Design and analyze a greedy approximation algorithm for the art gallery problem. What approximation ratio can you achieve?

\item For the fortress problem (guarding the exterior of a polygon), prove that $g_{\text{fortress}}(n) = \lceil n/2 \rceil$.

\item Investigate mobile guards that can patrol along edges. Prove that $g_{\text{mobile}}(n) = \lfloor n/4 \rfloor$.

\item Analyze the complexity of computing the exact chromatic number of planar graphs and its relationship to guard placement problems.
\end{enumerate}

\subsubsection{Construction and Diagram Questions}
\begin{enumerate}[start=31]
\item Draw two different planar embeddings of $K_4$. Label all vertices, edges, and faces, and verify Euler's formula for each.

\item Illustrate the comb polygon construction. Draw a comb with 16 vertices, place the optimal number of guards, and show why fewer guards are insufficient.

\item Create a planar graph with 7 vertices and exactly 12 edges. Verify that this achieves the maximum possible number of edges for a simple planar graph with 7 vertices.

\item Draw the wheel graph $W_6$ (cycle $C_6$ with central vertex). Determine if it's planar and find its chromatic number.

\item Construct a simple polygon and demonstrate the triangulation process step by step. Show the 3-coloring and resulting guard placement.

\item Illustrate why $K_5$ cannot be drawn without crossings by attempting different embeddings and showing where crossings are unavoidable.

\item Draw an orthogonal polygon with 12 vertices that requires exactly 3 guards. Show the guard placement and prove optimality.

\item Create a $y$-monotone polygon with 10 vertices. Find the minimum guard set and verify it matches the theoretical bound.

\item Design a star-shaped polygon and clearly indicate its kernel region. Show that one guard placed in the kernel can see the entire polygon.

\item Illustrate the fortress problem by drawing a polygon and showing how exterior guards must be placed to see the entire boundary from outside.
\end{enumerate}

\subsubsection{Proof-Based Questions}
\begin{enumerate}[start=41]
\item Prove that if $G$ is a connected planar graph with $n \geq 3$ vertices and $m$ edges, then $m \leq 3n - 6$.

\item Show that every planar graph contains a vertex of degree at most 5. Use this to prove that every planar graph is 6-colorable.

\item Prove that the complete graph $K_n$ is planar if and only if $n \leq 4$.

\item Demonstrate that the complete bipartite graph $K_{m,n}$ is planar if and only if $\min(m,n) \leq 2$.

\item For a simple polygon $P$, prove that any triangulation results in the same number of triangles and diagonals, regardless of the triangulation method chosen.

\item Show that the greedy algorithm for 3-coloring a triangulated polygon always produces a valid coloring and that one color class has size at most $\lfloor n/3 \rfloor$.

\item Prove that every quadrilateral (4-sided polygon) can be guarded by exactly one guard, regardless of its shape.

\item Show that for convex polygons with $n$ vertices, exactly one guard suffices and can be placed anywhere inside the polygon.

\item Prove that the visibility graph of any set of points in a convex polygon is the complete graph.

\item Demonstrate that determining the minimum number of guards for a specific polygon is equivalent to finding the minimum dominating set in its visibility graph.
\end{enumerate}

\subsubsection{Advanced Applications}
\begin{enumerate}[start=51]
\item Design an algorithm to determine if a given graph contains a $K_5$ or $K_{3,3}$ subdivision. Analyze its time complexity.

\item Investigate the relationship between the thickness of a graph (minimum number of planar graphs needed to cover all edges) and its planarity.

\item Explore the connection between graph coloring and art gallery problems. How does the chromatic number of the visibility graph relate to guard placement?

\item Study the dual graph of a planar graph. How do the properties of the original graph relate to its dual?

\item Analyze the art gallery problem for polygons with holes. How does the presence of holes affect the guard requirement bounds?

\item Examine rectilinear art gallery problems where guards and visibility are restricted to horizontal and vertical lines.

\item Investigate approximation algorithms for non-planar graphs. What bounds can be achieved for graphs embedded on surfaces of higher genus?

\item Study dynamic art gallery problems where the polygon can change over time. Design algorithms to maintain guard sets efficiently.

\item Explore probabilistic methods in graph coloring and their applications to randomized guard placement strategies.

\item Analyze the parameterized complexity of art gallery problems. What parameters make the problem tractable?
\end{enumerate}

\section{Network Flows and the Max-Flow Min-Cut Theorem}

This chapter develops, from first principles, every object and technique
needed to state and prove the Max-Flow Min-Cut Theorem of Ford and
Fulkerson. Up to this point the book has treated (mostly) undirected
graphs; flows live most naturally on directed graphs with non-negative
edge capacities, so we begin by introducing the setting carefully.

\subsection{Directed Graphs and Capacities}

\begin{definition}[Directed graph]
A \textbf{directed graph} (or \emph{digraph}) is a pair $D = (V, E)$
where $V$ is a finite set of \emph{vertices} and $E \subseteq V \times V$
is a set of ordered pairs called \emph{arcs} (or directed edges). If
$e = (u, v) \in E$, we call $u$ the \emph{tail} and $v$ the \emph{head}
of $e$, and we write $u \to v$.
\end{definition}

\begin{definition}[In- and out-neighborhoods]
For $v \in V$,
\[
  N^+(v) = \{\, w \in V : (v, w) \in E \,\}, \qquad
  N^-(v) = \{\, u \in V : (u, v) \in E \,\}.
\]
The \emph{out-degree} and \emph{in-degree} of $v$ are
$d^+(v) = |N^+(v)|$ and $d^-(v) = |N^-(v)|$.
\end{definition}

\begin{definition}[Flow network]
A \textbf{flow network} is a tuple $N = (V, E, s, t, c)$ where
$(V, E)$ is a finite directed graph, $s, t \in V$ are two
distinguished vertices called the \emph{source} and \emph{sink}
(with $s \ne t$), and $c : E \to \mathbb{R}_{\ge 0}$ is a
\emph{capacity} function. We extend $c$ to all pairs by setting
$c(u, v) = 0$ whenever $(u, v) \notin E$.
\end{definition}

\begin{remark}
It is convenient to assume that $N$ contains no pair of
antiparallel arcs (i.e.\ both $(u, v)$ and $(v, u)$). Any network
can be converted into one with this property by subdividing one of
the two antiparallel arcs with a new intermediate vertex whose
incoming and outgoing arcs keep the original capacity. All theorems
of this chapter transfer unchanged.
\end{remark}

\subsection{Flows and the Value of a Flow}

\begin{definition}[Flow]
A \textbf{flow} in $N$ is a function $f : V \times V \to \mathbb{R}$
satisfying the following three properties:
\begin{enumerate}
  \item \emph{Capacity constraint:} $0 \le f(u, v) \le c(u, v)$ for
        all $(u, v)$.
  \item \emph{Skew symmetry (optional bookkeeping):} $f(u, v) = -f(v, u)$.
        (Many authors omit this and use only non-negative flows;
        both formulations are equivalent.)
  \item \emph{Conservation:} for every vertex $v \in V \setminus \{s, t\}$,
        \[
          \sum_{u \in V} f(u, v) \;=\; \sum_{w \in V} f(v, w).
        \]
        That is, total flow into $v$ equals total flow out of $v$.
\end{enumerate}
\end{definition}

\begin{definition}[Value of a flow]
The \textbf{value} of a flow $f$ is the net flow leaving the source,
\[
...
\begin{remark}
By conservation, $|f|$ equals the net flow \emph{into} the sink:
\[
  |f| \;=\; \sum_{u \in V} f(u, t) \;-\; \sum_{w \in V} f(t, w).
\]
We prove this rigorously in Lemma~\ref{lem:netflow}.
\end{remark}

The \textbf{maximum-flow problem} asks: given a network $N$, find a
flow $f$ of largest possible value $|f|$.

\subsection{Cuts and the Capacity of a Cut}

\begin{definition}[$s$-$t$ cut]
An \textbf{$s$-$t$ cut} of $N$ is a partition $(S, T)$ of $V$ with
$s \in S$ and $t \in T$. The \textbf{capacity} of the cut is
\[
  c(S, T) \;=\; \sum_{u \in S} \sum_{v \in T} c(u, v),
\]
i.e.\ the total capacity of arcs that go from the $S$-side to the
$T$-side. (Arcs going from $T$ to $S$ are \emph{not} counted.)
\end{definition}

\begin{definition}[Flow across a cut]
For a flow $f$ and an $s$-$t$ cut $(S, T)$, the \textbf{net flow}
across the cut is
\[
...
The \textbf{minimum-cut problem} asks: find an $s$-$t$ cut $(S, T)$
of smallest possible capacity $c(S, T)$.

\subsection{Flow Value Equals Net Flow Across Any Cut}

\begin{lemma}[Flow across a cut]\label{lem:netflow}
...
\begin{proof}
Start from the definition of $|f|$ and add $0$'s using conservation.
For every $v \in S \setminus \{s\}$ we have
$\sum_{w} f(v, w) - \sum_{u} f(u, v) = 0$, so
\[
  |f|
  \;=\; \sum_{w} f(s, w) - \sum_{u} f(u, s)
  \;=\; \sum_{v \in S} \Big( \sum_{w \in V} f(v, w) - \sum_{u \in V} f(u, v) \Big).
\]
Expand the inner sums by splitting $V = S \cup T$:
\[
  = \sum_{v \in S}\sum_{w \in S} f(v, w)
    + \sum_{v \in S}\sum_{w \in T} f(v, w)
    - \sum_{v \in S}\sum_{u \in S} f(u, v)
    - \sum_{v \in S}\sum_{u \in T} f(u, v).
\]
The first and third double sums are equal (rename $(v, w) \leftrightarrow (u, v)$
inside $S \times S$) and cancel. What remains is exactly
$f(S, T)$.
\end{proof}

\begin{corollary}[Weak duality]\label{cor:weakduality}
For any flow $f$ and any $s$-$t$ cut $(S, T)$,
\[
  |f| \;\le\; c(S, T).
\]
In particular, $\max |f| \le \min c(S, T)$.
\end{corollary}

\begin{proof}
By Lemma~\ref{lem:netflow},
$|f| = \sum_{u \in S, v \in T} f(u, v) - \sum_{u \in S, v \in T} f(v, u)$.
The first sum is at most $\sum_{u \in S, v \in T} c(u, v) = c(S, T)$
by the capacity constraint, and the second sum is non-negative, so
$|f| \le c(S, T)$.
\end{proof}

Corollary~\ref{cor:weakduality} is already one half of the Max-Flow
Min-Cut Theorem: \emph{any} flow value is at most \emph{any} cut
capacity. The harder half is to show equality is attained. For that
we need the key construction of residual graphs.

\subsection{Residual Graphs and Augmenting Paths}

The idea: if we have a current flow $f$ and want to push more flow
along an arc $(u, v)$, we can either (a) use \emph{spare} capacity
$c(u, v) - f(u, v) > 0$, or (b) \emph{cancel} flow that was previously
sent from $v$ to $u$. Both options are encoded in a new graph.

\begin{definition}[Residual capacity]
Given a flow $f$ in $N$, the \textbf{residual capacity} is
\[
  c_f(u, v) \;=\; \big(c(u, v) - f(u, v)\big) \;+\; f(v, u),
\]
where we treat $c$ and $f$ as $0$ on non-existent arcs. The
\textbf{residual graph} is
\[
  N_f \;=\; (V, E_f), \qquad
  E_f \;=\; \{\, (u, v) : c_f(u, v) > 0 \,\}.
\]
...
So the residual graph may contain arcs not present in $E$ (these are
\emph{backward} arcs, corresponding to canceling existing flow) as
well as arcs from $E$ with leftover capacity.

\begin{definition}[Augmenting path]
An \textbf{augmenting path} (for the flow $f$) is a directed $s$-$t$
path in the residual graph $N_f$. Its \textbf{bottleneck}
$\Delta > 0$ is the minimum residual capacity of its arcs:
\[
  \Delta \;=\; \min_{(u, v) \in P} c_f(u, v).
\]
\end{definition}

\begin{lemma}[Pushing along an augmenting path]\label{lem:augment}
Let $P$ be an augmenting path in $N_f$ with bottleneck $\Delta$.
...
\[
  ...
  \end{cases}
\]
Then $f'$ is a flow in $N$ with $|f'| = |f| + \Delta$.
\end{lemma}

\begin{proof}
\emph{Capacity.} If we pushed forward on $(u, v) \in E$, then
$f'(u, v) = f(u, v) + \Delta \le f(u, v) + (c(u, v) - f(u, v)) = c(u, v)$
because $\Delta \le c_f(u, v) \le c(u, v) - f(u, v)$ is ensured by
the definition of $c_f$ when the path uses the ``forward'' part. If
we pushed backward, $f'(u, v) = f(u, v) - \Delta \ge 0$ for the same
reason. In either case capacity is preserved.

\emph{Conservation.} At every internal vertex $v$ on $P$, exactly one
arc of $P$ enters $v$ and exactly one leaves. In each of the four
combinations (forward/forward, forward/backward, etc.) the net change
at $v$ is $0$: the contribution added on one side is matched by an
identical contribution on the other side. For vertices not on $P$,
nothing changes.

\emph{Value.} At the source $s$, exactly one arc of $P$ is incident,
and it is directed away from $s$ in $N_f$. If that arc is a forward
arc $(s, w) \in E$, then $|f'| = |f| + \Delta$. If it is a backward
arc, i.e.\ $(w, s) \in E$ with flow being canceled, then
$\sum_u f'(u, s) = \sum_u f(u, s) - \Delta$, which again yields
$|f'| = |f| + \Delta$.
\end{proof}

\subsection{The Max-Flow Min-Cut Theorem}

\begin{theorem}[Ford--Fulkerson, 1956]\label{thm:mfmc}
Let $N = (V, E, s, t, c)$ be a flow network with a flow $f$. The
following are equivalent:
\begin{enumerate}
  \item $f$ is a maximum flow in $N$.
  \item The residual graph $N_f$ contains no augmenting path
        (i.e.\ no directed $s \to t$ path).
  \item There exists an $s$-$t$ cut $(S, T)$ with
        $|f| = c(S, T)$.
\end{enumerate}
In particular, the value of a maximum flow equals the capacity of a
minimum $s$-$t$ cut:
\[
...
\begin{proof}
\emph{(1) $\Rightarrow$ (2).} If $N_f$ contained an augmenting path
with bottleneck $\Delta > 0$, Lemma~\ref{lem:augment} produces a flow
of value $|f| + \Delta > |f|$, contradicting maximality.

\emph{(2) $\Rightarrow$ (3).} Suppose no augmenting path exists. Let
\[
  S \;=\; \{\, v \in V : \text{there is a directed path from } s \text{ to } v \text{ in } N_f \,\},
  \qquad T = V \setminus S.
\]
Then $s \in S$ by reflexivity; $t \in T$ because no $s$-$t$ path
exists in $N_f$. So $(S, T)$ is a valid $s$-$t$ cut. For every
$u \in S$ and $v \in T$, the arc $(u, v)$ is \emph{not} in $E_f$
(otherwise $v$ would be reachable from $s$), so $c_f(u, v) = 0$,
which means
\[
  \big(c(u, v) - f(u, v)\big) + f(v, u) \;=\; 0.
\]
Since both summands are non-negative, both must be zero:
$f(u, v) = c(u, v)$ and $f(v, u) = 0$. Summing over $u \in S$,
$v \in T$ and using Lemma~\ref{lem:netflow},
\[
  |f| \;=\; \sum_{u \in S, v \in T} f(u, v) - \sum_{u \in S, v \in T} f(v, u)
       \;=\; \sum_{u \in S, v \in T} c(u, v) - 0
       \;=\; c(S, T).
\]

\emph{(3) $\Rightarrow$ (1).} By weak duality
(Corollary~\ref{cor:weakduality}), $|f| \le c(S, T)$ for every cut,
so any flow whose value already equals \emph{some} cut capacity is
already as large as possible.
\end{proof}

\subsection{The Ford--Fulkerson Algorithm}

Theorem~\ref{thm:mfmc} suggests an algorithm: starting from the zero
flow, repeatedly find an augmenting path and push flow along it,
until no augmenting path remains. When the algorithm terminates, the
set $S$ of vertices reachable from $s$ in the final residual graph
gives a minimum cut.

\begin{algorithm}
\caption{Ford--Fulkerson}
\begin{algorithmic}[1]
\State $f(u, v) \gets 0$ for every arc $(u, v)$.
\While{there exists a directed $s$-$t$ path $P$ in $N_f$}
  \State $\Delta \gets \min_{(u, v) \in P} c_f(u, v)$
  \ForAll{$(u, v) \in P$}
    \If{$(u, v) \in E$} $f(u, v) \gets f(u, v) + \Delta$
    \Else{ } $f(v, u) \gets f(v, u) - \Delta$
    \EndIf
  \EndFor
\EndWhile
\State \Return $f$
...
If all capacities are integers, then at every iteration the flow
values remain integers, and the algorithm terminates in at most
$|f^*|$ iterations, where $f^*$ is a maximum flow. Consequently,
some maximum flow is integer-valued.
\end{theorem}

\begin{proof}
By induction: initially the flow is $0 \in \mathbb{Z}$. If $f$ is
integer-valued then every residual capacity is an integer, so the
bottleneck $\Delta$ is a positive integer, so the new flow $f'$ is
integer-valued. Each iteration increases $|f|$ by at least $1$, and
$|f|$ is bounded above by the capacity of any cut, hence finite.
\end{proof}

\begin{remark}[Termination with real capacities]
For irrational capacities the naive algorithm may fail to terminate
(Ford and Fulkerson gave an explicit example). The Edmonds--Karp
refinement---always choose a \emph{shortest} augmenting path (fewest
arcs) via BFS---terminates in $O(|V| \cdot |E|^2)$ time regardless of
capacities; Dinic's algorithm brings this down to $O(|V|^2 |E|)$. We
do not prove these bounds here, but they rely on the same residual
graph machinery introduced above.
\end{remark}

\subsection{Applications}

The Max-Flow Min-Cut Theorem is a workhorse. We derive two classical
consequences.

\subsubsection{Menger's Theorem}

\begin{theorem}[Menger, edge version]
Let $G$ be a finite directed graph and $s, t$ two distinct vertices.
The maximum number of pairwise edge-disjoint directed $s$-$t$ paths
equals the minimum number of arcs whose removal disconnects $t$
from $s$.
\end{theorem}

\begin{proof}[Sketch]
Turn $G$ into a flow network by giving every arc capacity $1$. By
Theorem~\ref{thm:integrality} there is an integer maximum flow $f^*$;
since capacities are $1$, flow on each arc is either $0$ or $1$. The
arcs carrying flow decompose into $|f^*|$ edge-disjoint $s$-$t$ paths
(remove them greedily, using conservation at every internal vertex).
Conversely, a minimum cut is a set of arcs of total capacity
$|f^*|$---i.e.\ a set of $|f^*|$ arcs whose removal puts $t$ on a
different side of the cut than $s$, disconnecting them. Equality is
Theorem~\ref{thm:mfmc}.
\end{proof}

A vertex version of Menger's theorem follows by the standard
\emph{vertex splitting} reduction: replace each internal vertex $v$
by two copies $v^{\text{in}}, v^{\text{out}}$ joined by an arc of
capacity $1$, and replicate incoming/outgoing arcs accordingly.

\subsubsection{König's Theorem Revisited}

\begin{theorem}[König]
In any bipartite graph $G = (A \cup B, E)$, the size of a maximum
matching equals the size of a minimum vertex cover.
\end{theorem}

\begin{proof}
Build a network: add source $s$ with arcs $s \to a$ (capacity $1$)
for each $a \in A$, add sink $t$ with arcs $b \to t$ (capacity $1$)
for each $b \in B$, and for every edge $\{a, b\} \in E$ add an arc
$a \to b$ of capacity $\infty$. By Theorem~\ref{thm:integrality}
there is an integer max flow $f^*$, and the arcs $a \to b$ with
$f^*(a, b) = 1$ form a matching of size $|f^*|$.

Let $(S, T)$ be a minimum cut. Since arcs $a \to b$ have capacity
$\infty$, no such arc may cross from $S$ to $T$ in a finite cut, so
whenever $a \in A \cap S$ and $\{a, b\} \in E$ we have $b \in B \cap S$.
Therefore
\[
  C \;=\; (A \cap T) \;\cup\; (B \cap S)
\]
is a vertex cover. Its size is
$|A \cap T| + |B \cap S| = c(S, T) = |f^*|$, matching the size of
the maximum matching. Weak duality (a cover has size at least the
size of any matching because each matching edge needs a cover vertex)
...
\begin{example}
Consider the network with vertices $\{s, a, b, t\}$ and arcs
\[
  s \to a\ (3),\quad s \to b\ (2),\quad
  a \to b\ (1),\quad a \to t\ (2),\quad b \to t\ (3).
\]
The diagram below shows the network, the final flow, and the
minimum cut.
...
\begin{tikzpicture}[
    scale=1.0,
    vertex/.style={circle, draw, fill=white, minimum size=0.7cm, inner sep=1pt},
    flowedge/.style={-{Stealth[length=3mm]}, thick},
    cutline/.style={dashed, color=red, line width=1.0pt}
]
    \node[vertex] (s) at (0,0) {$s$};
    \node[vertex] (a) at (3,1.2) {$a$};
    \node[vertex] (b) at (3,-1.2) {$b$};
    \node[vertex] (t) at (6,0) {$t$};

    \draw[flowedge, color=blue] (s) -- node[above]{\small $2/3$} (a);
    \draw[flowedge, color=blue] (s) -- node[below]{\small $2/2$} (b);
    \draw[flowedge, color=blue] (a) -- node[right]{\small $1/1$} (b);
    \draw[flowedge, color=blue] (a) -- node[above]{\small $2/2$} (t);
    \draw[flowedge, color=blue] (b) -- node[below]{\small $3/3$} (t);

    \draw[cutline] (1.0,2.0) -- (1.0,-2.0);
    \node[red] at (1.0,2.4) {\small cut $(S,T)$};

    \node at (3,-2.3) {\small Edge labels: flow$/$capacity. Cut capacity $= 3 + 2 = 5$.};
\end{tikzpicture}
\\
\small Figure 200: A flow network of value $5$ with the saturating min cut $S = \{s\}$.
\label{fig:mfmc-example}
\end{figure}

One augmenting path is $s \to a \to t$ with bottleneck $2$; push $2$
units. Next, $s \to b \to t$ has bottleneck $2$; push $2$ more.
Residual graph now has $s \to a\ (1)$, $a \to b\ (1)$, $b \to t\ (1)$,
yielding the path $s \to a \to b \to t$ with bottleneck $1$; push
another unit. No augmenting path remains: the set of vertices
reachable from $s$ is $S = \{s\}$, giving cut $(\{s\}, \{a, b, t\})$
of capacity $3 + 2 = 5$. Max flow value is $2 + 2 + 1 = 5 = c(S, T)$,
confirming Theorem~\ref{thm:mfmc}.
\end{example}
...
\begin{exercise}
Prove the ``other half'' of Lemma~\ref{lem:netflow}: for any flow
$f$, the net flow into the sink $t$ equals $|f|$.
\end{exercise}

\begin{exercise}
Derive the vertex form of Menger's theorem from the edge form using
the vertex-splitting construction described above.
\end{exercise}

\begin{exercise}
On the network of Figure~\ref{fig:mfmc-example}, list all augmenting
paths the algorithm could explore in some order, and verify that the
final flow value is independent of the order.
\end{exercise}

\subsubsection{Medium Questions}

\begin{exercise}
Let $N$ be a flow network with integer capacities bounded by $C$ and
$m$ arcs. Show that the Ford--Fulkerson algorithm (using any
augmenting paths) runs in $O(m C |V|)$ time. Give an example where
this bound is tight up to constants.
\end{exercise}

\begin{exercise}
Use the max-flow min-cut theorem to prove Hall's marriage theorem:
a bipartite graph $G = (A \cup B, E)$ has a matching saturating $A$
iff $|N(S)| \ge |S|$ for every $S \subseteq A$.
\end{exercise}

\begin{exercise}
Given an undirected graph $G$ with edge capacities, define the
\emph{global minimum cut} as the partition $(S, V \setminus S)$ with
$\emptyset \ne S \ne V$ minimizing the total capacity of edges
crossing the cut. Show how to compute it with $|V| - 1$ calls to a
max-flow subroutine.
\end{exercise}
...
\begin{exercise}
Construct an example with \emph{irrational} capacities on which a
particular choice of augmenting paths causes Ford--Fulkerson to
run forever while the flow value converges to a number strictly
smaller than the true maximum. (This is the classical Ford--Fulkerson
counterexample; it motivates the Edmonds--Karp rule.)
\end{exercise}

\begin{exercise}
Prove the Edmonds--Karp bound: if augmenting paths are always chosen
by BFS (shortest first), Ford--Fulkerson terminates after at most
$O(|V| \cdot |E|)$ augmentations and runs in $O(|V| \cdot |E|^2)$
time, regardless of capacities.
\end{exercise}

\subsubsection{Diagram Questions}

\begin{exercise}
Draw a flow network on $\{s, x, y, z, t\}$ with five arcs of capacity
at most $4$ such that the max-flow value equals $3$ and the
min-cut consists of exactly two arcs. Mark the saturating cut.
\end{exercise}

\begin{exercise}
For the diamond network with vertices $\{s, u, v, t\}$ and arcs
$s\to u\,(10),\ s\to v\,(10),\ u\to v\,(1),\ u\to t\,(10),\ v\to t\,(10)$,
draw the residual graph after pushing $10$ units along $s \to u \to t$
and identify the next augmenting path.
\end{exercise}

\section{Advanced Topics in Graph Theory}

The remainder of the book is a tour of topics commonly taught in
first-year graduate courses on graph theory and combinatorics. Each
subsection is self-contained modulo the definitions established in
earlier chapters, but together they sketch the modern toolkit:
\emph{extremal} arguments (forbidden-subgraph problems),
\emph{Ramsey-type} unavoidability, the \emph{probabilistic method},
\emph{spectral} (linear-algebraic) techniques, \emph{random graph}
models, Szemer\'edi's \emph{regularity lemma}, and the
\emph{graph minor} programme of Robertson and Seymour.

\subsection{Extremal Graph Theory}

Extremal graph theory studies the maximum (or minimum) possible value
of some graph parameter (typically edges) among graphs satisfying a
given constraint (typically excluding a fixed subgraph $H$). The
central object is the Tur\'an number $\mathrm{ex}(n, H)$. This
subsection develops the non-bipartite theory completely (Tur\'an,
Erd\H{o}s--Simonovits stability, Erd\H{o}s--Stone) and then treats
the much subtler bipartite theory, including the K\H{o}v\'ari--S\'os--Tur\'an
theorem, the Bondy--Simonovits even-cycle bound, the Zarankiewicz
problem, supersaturation, and the Moore bound for graphs of high girth.

\subsubsection{The Tur\'an Number and the Tur\'an Graph}

\begin{definition}[Extremal number]\label{def:exnumber}
For a graph $H$ and integer $n \ge 1$, the \textbf{extremal number}
(or \emph{Tur\'an number}) is
\[
  \mathrm{ex}(n, H) \;=\; \max\{\, |E(G)| : |V(G)| = n,\ G \text{ contains no copy of } H \,\}.
\]
A graph attaining this maximum is called $H$-\emph{extremal}. The
density version is $\pi(H) = \lim_{n \to \infty} \mathrm{ex}(n, H) / \binom{n}{2}$,
called the \textbf{Tur\'an density} of $H$ (the limit exists by an
averaging argument of Katona, Nemetz, and Simonovits).
\end{definition}

\begin{definition}[Tur\'an graph]\label{def:turangraph}
The \textbf{Tur\'an graph} $T(n, r)$ is the complete $r$-partite graph
on $n$ vertices whose part sizes differ by at most one (a
\emph{balanced} $r$-partition). Its edge count is
\[
  t(n, r) \;=\; \binom{n}{2} - \sum_{i=1}^{r} \binom{|V_i|}{2}
           \;=\; \Big(1 - \tfrac{1}{r}\Big)\frac{n^2}{2} + O(1).
\]
\end{definition}
...
\begin{tikzpicture}[
    scale=0.95,
    vertex/.style={circle, draw, fill=white, minimum size=0.45cm, inner sep=1pt},
    redge/.style={line width=0.5pt, color=gray!70}
]
    % T(7,3): parts of sizes 3, 2, 2
    \foreach \i/\x in {1/-2.5, 2/-2.5, 3/-2.5} {
        \pgfmathsetmacro{\y}{1.4 - 1.4*(\i-1)}
        \node[vertex, fill=red!25] (A\i) at (\x,\y) {};
    }
    \foreach \i/\x in {1/0, 2/0} {
        \pgfmathsetmacro{\y}{0.7 - 1.4*(\i-1)}
        \node[vertex, fill=green!25] (B\i) at (\x,\y) {};
    }
    \foreach \i/\x in {1/2.5, 2/2.5} {
        \pgfmathsetmacro{\y}{0.7 - 1.4*(\i-1)}
        \node[vertex, fill=blue!25] (C\i) at (\x,\y) {};
    }
    % All edges between different parts
    \foreach \a in {1,2,3} \foreach \b in {1,2} \draw[redge] (A\a) -- (B\b);
    \foreach \a in {1,2,3} \foreach \b in {1,2} \draw[redge] (A\a) -- (C\b);
    \foreach \b in {1,2} \foreach \c in {1,2} \draw[redge] (B\b) -- (C\c);

    \node at (-2.5,-2.4) {\small Part $V_1$};
    \node at (0,-2.4) {\small Part $V_2$};
    \node at (2.5,-2.4) {\small Part $V_3$};
    \node at (0,-3.2) {\small $T(7,3)$: balanced complete $3$-partite, $K_4$-free, $|E| = 16$};
\end{tikzpicture}
\\
\small Figure 201: The Tur\'an graph $T(7,3)$ achieves $\mathrm{ex}(7, K_4)$.
\label{fig:turan-graph}
\end{figure}

\subsubsection{Mantel's Theorem: Three Proofs}

\begin{theorem}[Mantel, 1907]\label{thm:mantel}
$\mathrm{ex}(n, K_3) = \big\lfloor n^2/4 \big\rfloor$, attained uniquely
(up to isomorphism) by the balanced complete bipartite graph
$K_{\lceil n/2\rceil, \lfloor n/2\rfloor}$.
\end{theorem}

\begin{proof}[Proof 1: local neighborhoods and Cauchy--Schwarz]
Let $G$ be triangle-free with $n$ vertices and $m$ edges. For any edge
$uv \in E(G)$, the sets $N(u)$ and $N(v)$ must be disjoint, as a common
neighbor $w$ would produce triangle $uvw$. Therefore
$d(u) + d(v) \le n$. Summing over all edges,
\[
  \sum_{uv \in E} \big(d(u) + d(v)\big) \;=\; \sum_{v \in V} d(v)^2
  \;\le\; n \cdot m.
\]
By the Cauchy--Schwarz inequality $\sum_v d(v)^2 \ge (\sum_v d(v))^2 / n = (2m)^2 / n$.
Combining, $4m^2/n \le nm$, hence $m \le n^2/4$.
\end{proof}

\begin{proof}[Proof 2: induction on $n$]
The inequality $m \le \lfloor n^2/4 \rfloor$ is trivial for $n \le 2$.
Suppose it holds for $n - 2$ and let $G$ be triangle-free on $n$
vertices. If $G$ has no edges we are done; otherwise pick any edge
$uv$. Write $G' = G \setminus \{u, v\}$, which has at most
$\lfloor (n-2)^2/4 \rfloor$ edges by induction. Triangle-freeness and
the disjointness $N(u) \cap N(v) = \emptyset$ show that
$u$ and $v$ together contribute $d(u) + d(v) - 1 \le n - 1$ edges, so
\[
  m \;\le\; \Big\lfloor \tfrac{(n-2)^2}{4} \Big\rfloor + (n - 1)
     \;=\; \Big\lfloor \tfrac{n^2}{4} \Big\rfloor.
\]
\end{proof}

\begin{proof}[Proof 3: weight-shifting / LP relaxation]
Assign each vertex $v$ weight $x_v \ge 0$ with $\sum x_v = 1$. Define
$f(\mathbf{x}) = \sum_{uv \in E} x_u x_v$. Triangle-freeness makes
$f$ at most $1/4$: indeed, if $x_u > 0$ and $x_v > 0$ for some edge
$uv$, either $x_u > x_v$ (shift all weight of $v$ onto $u$,
increasing $f$) or $x_v \ge x_u$ (shift the other way), so the max
is at a two-vertex support, giving $f \le \max_{t \in [0,1]} t(1 - t) = 1/4$.
Now set $x_v = 1/n$ to obtain $m/n^2 \le 1/4$.
\end{proof}

\begin{remark}
Proof~3 is the weighted-density technique central to flag-algebra
arguments; it generalizes immediately to any forbidden subgraph.
Proof~1 generalizes to a fractional ``number-of-edges in triangle''
count used in Goodman's theorem (below).
\end{remark}

\begin{theorem}[Goodman, 1959]
Every graph $G$ on $n$ vertices with $m$ edges contains at least
\[
  \frac{m}{3}\big( 4m/n - n \big)
\]
triangles. In particular a triangle-free graph has $m \le n^2/4$.
\end{theorem}

\begin{proof}
Count, for each vertex $v$, the number of \emph{cherry} subgraphs
(paths of length $2$) with $v$ as endpoint: this is $\binom{d(v)}{2}$
but also counts, for each non-edge $xy$ with $v \in N(x) \cap N(y)$,
one cherry. Each triangle is counted thrice, so the number of
triangles is
\[
  T = \tfrac{1}{3}\Big( \sum_v \binom{d(v)}{2} - \# (\text{paths ending in non-edges}) \Big).
\]
An application of Cauchy--Schwarz produces the stated lower bound.
\end{proof}

\subsubsection{Tur\'an's Theorem}

\begin{theorem}[Tur\'an, 1941]\label{thm:turan}
For $r \ge 2$ and $n \ge r$, if $G$ is a graph on $n$ vertices with
no $K_{r+1}$ subgraph, then $|E(G)| \le t(n, r)$, with equality
only when $G \cong T(n, r)$.
\end{theorem}

\begin{proof}[Proof 1: Zykov symmetrization]
Let $G$ be $K_{r+1}$-free with the maximum number of edges among all
$K_{r+1}$-free graphs on $n$ vertices. We show that ``not adjacent''
is an equivalence relation on $V(G)$.

\emph{Symmetry} and \emph{reflexivity} are automatic. For
\emph{transitivity}, suppose $u \not\sim v$ and $v \not\sim w$ but
$u \sim w$. Without loss of generality $d(u) \ge d(v)$.
Delete $v$ and replace it by a clone $v'$ of $u$ with $N(v') = N(u)$.
The new graph is still $K_{r+1}$-free (a clique through $v'$ would
give a clique through $u$), and its edge count is
$|E| - d(v) + d(u) \ge |E|$. In fact the inequality is strict unless
$d(u) = d(v)$, but even when equal, iterating this move eventually
makes all three vertices mutually non-adjacent, proving transitivity
up to edge-preserving operations.

Thus in the extremal case $V(G)$ splits into classes of mutually
non-adjacent vertices, with all edges between distinct classes
present. That is, $G$ is complete multipartite with some number of
parts $s \le r$. Among complete $s$-partite graphs with
$s$ parts of total size $n$, the edge count
$\binom{n}{2} - \sum \binom{|V_i|}{2}$ is maximized (by convexity of
$x \mapsto \binom{x}{2}$) when the parts are as equal as possible.
Among $s \in \{1, \dots, r\}$, the choice $s = r$ dominates, giving
$T(n, r)$.
\end{proof}

\begin{proof}[Proof 2: Motzkin--Straus (probabilistic / LP)]
Define the \emph{Lagrangian}
\[
  \lambda(G) \;=\; \max\Big\{\, \sum_{uv \in E} x_u x_v : x \ge 0,\ \textstyle\sum_v x_v = 1 \,\Big\}.
\]
For any graph $G$ on $n$ vertices, setting $x_v = 1/n$ gives
$\lambda(G) \ge m/n^2$. Motzkin and Straus proved
$\lambda(G) = \tfrac{1}{2}(1 - 1/\omega(G))$, where $\omega(G)$ is the
clique number. If $\omega(G) \le r$, then
$m/n^2 \le \tfrac{1}{2}(1 - 1/r)$, giving Tur\'an's bound. The key to
Motzkin--Straus is to observe that any optimal $\mathbf x$ is
supported on a clique of $G$.
\end{proof}

\begin{proof}[Proof 3: induction via a max-degree vertex]
Let $v$ be a vertex of maximum degree $\Delta$ in a $K_{r+1}$-free
graph $G$. Write $N = N(v)$, $M = V \setminus (N \cup \{v\})$. The
induced subgraph $G[N]$ is $K_r$-free (a $K_r$ in $G[N]$ plus $v$
forms $K_{r+1}$), so by induction $|E(G[N])| \le t(\Delta, r - 1)$.
Edges not inside $N$ total at most $\Delta \cdot (n - \Delta)$ (the
max-degree bound). Combining and optimizing over $\Delta$ yields the
result.
\end{proof}

\begin{corollary}[Tur\'an density of a clique]
$\pi(K_{r+1}) = 1 - 1/r$.
\end{corollary}

\begin{corollary}[Chromatic consequence]
Every graph $G$ on $n$ vertices with $|E(G)| > t(n, r)$ edges
contains a $K_{r+1}$; equivalently, any $G$ with $|E(G)| > (1 - 1/r) n^2/2$
has chromatic number at least $r + 1$.
\end{corollary}

\subsubsection{Stability: The Erd\H{o}s--Simonovits Theorem}

A hallmark of modern extremal graph theory: if a graph is $K_{r+1}$-free
with \emph{nearly} extremal edge count, it must itself look almost
like $T(n, r)$.

\begin{theorem}[Erd\H{o}s--Simonovits stability, 1968]\label{thm:stability}
For every $r \ge 2$ and $\varepsilon > 0$ there exist $\delta > 0$ and
$n_0$ such that any $K_{r+1}$-free graph $G$ on $n \ge n_0$ vertices
with $|E(G)| \ge t(n, r) - \delta n^2$ edges can be transformed into
$T(n, r)$ by adding or deleting at most $\varepsilon n^2$ edges.
\end{theorem}

\begin{proof}[Proof sketch]
If $G$ were far from $T(n, r)$ in edit distance, the Zykov
symmetrization of Proof~1 of Theorem~\ref{thm:turan} would be
strictly improving, contradicting extremality up to $\delta n^2$
edges. A full proof uses the Szemer\'edi regularity lemma (see the
subsection on regularity) to replace ``close to $T(n, r)$'' by a
regularity-partition statement and then extracts a Tur\'an-type
improvement.
\end{proof}

\begin{remark}
Stability statements are the workhorse of ``solve by perturbation''
arguments: prove the result for the extremal configuration, then
extend by smallness of perturbations.
\end{remark}

\subsubsection{The Erd\H{o}s--Stone--Simonovits Theorem}

\begin{theorem}[Erd\H{o}s--Stone, 1946; Erd\H{o}s--Simonovits, 1966]
For any graph $H$ with chromatic number $\chi(H) \ge 2$,
\[
  \mathrm{ex}(n, H) \;=\; \Big(1 - \tfrac{1}{\chi(H) - 1} + o(1)\Big)\frac{n^2}{2}
  \quad\text{as } n \to \infty.
\]
Equivalently $\pi(H) = 1 - 1/(\chi(H) - 1)$.
\end{theorem}

\begin{proof}[Proof sketch via Erd\H{o}s--Stone]
The Erd\H{o}s--Stone theorem asserts that for every $r \ge 2$, every
$t \ge 1$, and $\varepsilon > 0$, if $n$ is large enough then every graph
on $n$ vertices with $|E(G)| \ge (1 - 1/r + \varepsilon) n^2/2$ edges
contains the complete $(r+1)$-partite graph $K_{r+1}(t)$ with each
part of size $t$. Given $H$ of chromatic number $r + 1$, choose $t$
large enough that $H \subseteq K_{r+1}(t)$. Any graph with more than
$(1 - 1/r + \varepsilon) n^2/2$ edges therefore contains $K_{r+1}(t) \supseteq H$.
Combining with Tur\'an's lower bound $t(n, r) \le \mathrm{ex}(n, H)$
(since $T(n, r)$ is $(r+1)$-chromatic-free, thus $H$-free) gives the
theorem.
\end{proof}

Two lessons from this theorem.

\begin{remark}[The bipartite cliff]
For bipartite $H$ we have $\chi(H) = 2$, so Erd\H{o}s--Stone merely
gives $\mathrm{ex}(n, H) = o(n^2)$---the leading order vanishes and we
must determine the exact exponent of $n$. This is the central
difficulty in the \emph{degenerate} (bipartite) theory.
\end{remark}

\begin{remark}[Supersaturation]
Erd\H{o}s and Simonovits also proved that exceeding $\mathrm{ex}(n, H)$
by a constant fraction yields not merely one but $\Theta(n^{|V(H)|})$
copies of $H$: the dependence is \emph{supersaturated}. This is a
quantitative strengthening routinely exploited.
\end{remark}

\subsubsection{The Bipartite Case: K\H{o}v\'ari--S\'os--Tur\'an}

\begin{theorem}[K\H{o}v\'ari--S\'os--Tur\'an, 1954]\label{thm:kst}
For integers $2 \le s \le t$,
\[
  \mathrm{ex}(n, K_{s, t}) \;\le\; \tfrac{1}{2}(t - 1)^{1/s}\big(n^{2 - 1/s}\big) + \tfrac{s - 1}{2} n.
\]
\end{theorem}

\begin{proof}
Double-count the number of ``cherries'' $S = \{A \subseteq V(G) : |A| = s,\ A \subseteq N(v) \text{ for some } v\}$.
For each $v \in V(G)$ the number of $s$-subsets of $N(v)$ is
$\binom{d(v)}{s}$, so
\[
  \sum_{v \in V} \binom{d(v)}{s} \;=\; \#\{(A, v) : |A| = s,\ A \subseteq N(v)\}.
\]
If $G$ contains no $K_{s, t}$, then each $s$-set $A$ has at most
$t - 1$ common neighbors $v$, so the right-hand side is at most
$(t - 1)\binom{n}{s}$. By convexity of $\binom{x}{s}$,
\[
  n \binom{2m/n}{s} \;\le\; \sum_v \binom{d(v)}{s} \;\le\; (t - 1)\binom{n}{s}.
\]
Solving for $m$, and using $\binom{x}{s} \ge (x - s + 1)^s / s!$,
gives the claimed bound.
\end{proof}

\begin{corollary}
$\mathrm{ex}(n, C_4) = \mathrm{ex}(n, K_{2, 2}) \le \tfrac{1}{2}(1 + \sqrt{4n - 3}) \cdot n / 2 = O(n^{3/2})$.
\end{corollary}

\begin{theorem}[Reiman; Erd\H{o}s--R\'enyi--S\'os]
The upper bound $\mathrm{ex}(n, C_4) \le \tfrac{1}{2}(1 + \sqrt{4n - 3})$
is attained to leading order: the incidence graph of the projective
plane $\mathrm{PG}(2, q)$ (a polarity graph) is $C_4$-free and has
$(1 + o(1)) n^{3/2}/2$ edges when $n = q^2 + q + 1$.
\end{theorem}

\begin{theorem}[Bondy--Simonovits, 1974]
For every integer $k \ge 2$,
\[
  \mathrm{ex}(n, C_{2k}) \;\le\; 100 k \cdot n^{1 + 1/k}.
\]
\end{theorem}

Whether this is tight for $k \ge 4$ is a long-standing open problem;
matching constructions are only known for $k \in \{2, 3, 5\}$
(projective plane, generalized quadrangle, generalized hexagon).

\subsubsection{The Zarankiewicz Problem}

\begin{definition}[Zarankiewicz function]
$z(m, n; s, t)$ is the maximum number of $1$'s in an $m \times n$
$0/1$-matrix with no all-ones $s \times t$ submatrix. Equivalently,
it is the maximum edge count of a $K_{s, t}$-free bipartite graph
with parts of sizes $m$ and $n$.
\end{definition}

\begin{theorem}[K\H{o}v\'ari--S\'os--Tur\'an, bipartite form]
$z(m, n; s, t) \le \tfrac{1}{2}\big((t - 1)^{1/s}(n - s + 1) m^{1 - 1/s} + (s - 1) m\big)$.
\end{theorem}

\subsubsection{Graphs of High Girth: The Moore Bound}

The interaction between extremal counts and girth is the complementary
theme: how few vertices can a graph with high minimum degree and
high girth have?

\begin{definition}[Girth]
The \textbf{girth} $g(G)$ is the length of the shortest cycle in $G$
(or $\infty$ if $G$ is a forest).
\end{definition}

\begin{theorem}[Moore bound]\label{thm:moore}
Let $G$ be a graph with minimum degree $\delta$ and girth at least
$2k + 1$. Then
\[
  |V(G)| \;\ge\; 1 + \delta + \delta(\delta - 1) + \cdots + \delta(\delta - 1)^{k - 1}.
\]
For even girth $2k$,
\[
  |V(G)| \;\ge\; 2\big(1 + (\delta - 1) + \cdots + (\delta - 1)^{k - 1}\big).
\]
\end{theorem}

\begin{proof}[Proof for odd girth]
Pick any vertex $v$ and perform BFS for $k$ layers. Girth $\ge 2k + 1$
ensures all vertices in layers $0, 1, \dots, k$ are distinct: a
repeat in layer $\le k$ would close a cycle of length $\le 2k$.
Each vertex in layer $i < k$ contributes $\ge \delta - 1$ neighbors
to layer $i + 1$ (except $v$ which contributes $\delta$ to layer $1$),
giving the stated count.
\end{proof}

\begin{definition}[Cage]
A $(\delta, g)$-\textbf{cage} is a $\delta$-regular graph of girth
$g$ with the smallest possible number of vertices.
\end{definition}

Cages attaining the Moore bound with equality are called
\emph{Moore graphs}. Famous examples include the Petersen graph
($(3, 5)$-cage, $10$ vertices), the Hoffman--Singleton graph
($(7, 5)$-cage, $50$ vertices), and the Heawood graph ($(3, 6)$-cage,
$14$ vertices).

\begin{theorem}[Hoffman--Singleton, 1960]
A Moore graph of girth $5$ and degree $\delta$ exists only if
$\delta \in \{2, 3, 7, 57\}$.
\end{theorem}

The $\delta = 57$ case remains open after more than sixty years---one
of the most famous unsolved problems in algebraic graph theory.

\subsubsection{Supersaturation}

\begin{theorem}[Erd\H{o}s--Simonovits supersaturation]
For every graph $H$ and $\varepsilon > 0$ there is $c > 0$ such that any
graph $G$ on $n$ vertices with $|E(G)| \ge (\pi(H) + \varepsilon) \binom{n}{2}$
edges contains at least $c \cdot n^{|V(H)|}$ copies of $H$.
\end{theorem}

\begin{proof}[Proof sketch]
Apply the regularity lemma to $G$ and pass to the cluster graph.
The regularity partition preserves the excess density, and the
counting lemma turns it into an embedding count of $H$.
\end{proof}
...
\begin{exercise}
Prove Mantel's theorem by induction on $n$: remove an edge $uv$ from
a triangle-free graph $G$, bound $|E(G - \{u, v\})|$ inductively, and
account for the removed degrees.
\end{exercise}

\begin{exercise}
Show that the Tur\'an graph $T(n, r)$ has chromatic number exactly
$r$. Deduce from Tur\'an's theorem the \emph{chromatic} density
bound: any graph on $n$ vertices with chromatic number at most $r$
has at most $t(n, r)$ edges.
\end{exercise}

\begin{exercise}
Compute the number of triangles in $T(n, r)$. For which $r$ does
$T(n, r)$ maximize the number of triangles among all $K_{r+1}$-free
...
\begin{exercise}
Prove that every $K_4$-free graph on $n$ vertices contains an
independent set of size $\ge \sqrt{n}$. (Hint: use Tur\'an's theorem
on the complement.)
\end{exercise}

\begin{exercise}
Prove the Erd\H{o}s--Ko--Rado-style bound
$\mathrm{ex}(n, K_{2, 2}) \le \tfrac{1}{2}(1 + \sqrt{4n - 3})$
directly, using the fact that any two vertices have at most one
common neighbor in a $K_{2, 2}$-free graph.
\end{exercise}

\begin{exercise}
Show that the incidence graph of a projective plane of order $q$
has $2(q^2 + q + 1)$ vertices, is $(q + 1)$-regular, and has girth
$6$. Verify that it achieves the Moore bound with equality.
\end{exercise}

\paragraph{Hard Questions.}

\begin{exercise}[Kruskal--Katona warm-up]
Let $G$ be a graph on $n$ vertices with $m$ edges. Show that the
number of triangles in $G$ is at most $\binom{m/(2^{1/2})}{3/2}$
asymptotically; deduce a direct upper bound for $\mathrm{ex}(n, K_4)$.
\end{exercise}

\begin{exercise}
Prove that every graph on $n$ vertices and at least $\lfloor n^2/4 \rfloor + 1$
edges contains at least $\lfloor n/2 \rfloor$ triangles. (This is
Rademacher's theorem, a supersaturation result extending Mantel.)
\end{exercise}

\begin{exercise}
Let $G$ be a graph with $|V(G)| = n$ and $\delta(G) \ge 2n/5$. Prove
that $G$ contains a triangle. Construct a graph showing that
$\delta \ge 2n/5$ cannot be weakened to $\delta \ge (2/5 - \varepsilon) n$.
\end{exercise}

\begin{exercise}[Erd\H{o}s--Gallai]
Show that every graph on $n$ vertices with more than
$\tfrac{1}{2}(k - 1) n$ edges contains a path of length $k$.
\end{exercise}

\paragraph{Diagram Questions.}

\begin{exercise}
Draw $T(8, 3)$ and $T(8, 4)$. For each, count the edges directly and
verify the formula $t(n, r) = \binom{n}{2} - \sum_i \binom{|V_i|}{2}$.
\end{exercise}

\begin{exercise}
Draw a $4$-regular graph on $10$ vertices with girth $5$ that achieves
the odd Moore bound $1 + 4 + 4 \cdot 3 = 17$ \emph{minus} the parity
gap (so on $10$ vertices). The Petersen graph is the canonical
example---draw it in the standard pentagon-pentagram form and label
the girth witness.
\end{exercise}

\subsection{Ramsey Theory}

Ramsey theory sits opposite extremal theory. Instead of asking how
dense a graph can be before it must contain $H$, we ask how large a
complete graph must be before \emph{every} red/blue coloring of its
edges contains a monochromatic copy of $H$. The unifying meta-theorem,
sometimes phrased as ``complete disorder is impossible'', asserts
that any sufficiently large structure contains a highly organized
substructure.

\subsubsection{The Classical Ramsey Numbers}

\begin{definition}[Ramsey number]\label{def:ramsey}
$R(s, t)$ is the least integer $N$ such that every red/blue coloring
of the edges of $K_N$ contains a red $K_s$ or a blue $K_t$. When
$s = t$ we write $R(s) := R(s, s)$ for the \emph{diagonal} Ramsey
number.
\end{definition}

\begin{definition}[Multicolor Ramsey number]
$R(s_1, \dots, s_k)$ is the least $N$ such that in any $k$-coloring of
the edges of $K_N$, some color class $i$ contains $K_{s_i}$.
\end{definition}

\begin{theorem}[Ramsey, 1930]\label{thm:ramsey}
For all $s, t \ge 2$, $R(s, t)$ is finite. Moreover
\[
  R(s, t) \;\le\; R(s - 1, t) + R(s, t - 1),
\]
and consequently $R(s, t) \le \binom{s + t - 2}{s - 1}$.
\end{theorem}

\begin{proof}
Induct on $s + t$, with base cases $R(2, t) = t$ and $R(s, 2) = s$
being immediate. For the inductive step, pick any vertex $v$ in
$K_N$ with $N = R(s - 1, t) + R(s, t - 1)$. Among its $N - 1$ incident
edges, either $\ge R(s - 1, t)$ are red or $\ge R(s, t - 1)$ are blue
(pigeonhole). In the first case, apply induction to the induced red
neighborhood to obtain either a red $K_{s - 1}$ (which together with
$v$ gives a red $K_s$) or a blue $K_t$; the second case is symmetric.

For the closed-form bound, one verifies
$\binom{(s-1) + t - 2}{s - 2} + \binom{s + (t-1) - 2}{s - 1} = \binom{s + t - 2}{s - 1}$
and concludes by induction using Pascal's identity.
\end{proof}

\begin{corollary}[Diagonal upper bound]
$R(k) \le \binom{2k - 2}{k - 1} \sim \dfrac{4^{k - 1}}{\sqrt{\pi k}}$.
\end{corollary}

\begin{theorem}[Erd\H{o}s--Szekeres refinement]
When $s + t$ is odd, the bound above is strict: $R(s, t) \le \binom{s + t - 2}{s - 1} - 1$.
\end{theorem}

\begin{remark}[Small cases]
The following exact values are known, largely by exhaustive computer
search:
\[
...
\]
The value of $R(5, 5)$ is unknown, with $43 \le R(5) \le 46$ as of
2025. Erd\H{o}s's famous quip: if aliens demanded the exact value of
$R(5, 5)$, we should devote every computer on Earth to it; if they
demanded $R(6, 6)$, we should attack them first.
\end{remark}

\begin{theorem}[$R(3, 3) = 6$]\label{thm:r33}
In any red/blue coloring of the edges of $K_6$ there is a monochromatic
triangle; $K_5$ admits a coloring with no monochromatic triangle (the
pentagon plus pentagram coloring).
\end{theorem}

\begin{proof}
Pick any vertex $v$ of $K_6$: among its $5$ incident edges, by
pigeonhole at least three, say $vu_1, vu_2, vu_3$, have the same
color, say red. If any $u_i u_j$ is red we have a red triangle; if
all three are blue we have a blue triangle $u_1 u_2 u_3$. For
$K_5$, color the $5$-cycle edges red and the ``chord'' edges of the
pentagram blue; the only triangles of $K_5$ use both red and blue
edges.
\end{proof}
...
\begin{tikzpicture}[
    scale=0.95,
    vertex/.style={circle, draw, fill=white, minimum size=0.45cm, inner sep=1pt},
    rede/.style={line width=0.9pt, color=red},
    blue/.style={line width=0.9pt, color=blue}
]
    % LEFT: K_5 with no monochromatic triangle
    \begin{scope}[xshift=-3.5cm]
        \foreach \i in {1,2,3,4,5} {
            \node[vertex] (v\i) at ({90 + 72*(\i-1)}:1.6) {\small $\i$};
        }
        % Pentagon (cycle): red
        \draw[rede] (v1) -- (v2);
        \draw[rede] (v2) -- (v3);
        ...
        \draw[rede] (v4) -- (v5);
        \draw[rede] (v5) -- (v1);
        % Pentagram (chords): blue
        \draw[blue] (v1) -- (v3);
        \draw[blue] (v2) -- (v4);
        \draw[blue] (v3) -- (v5);
        \draw[blue] (v4) -- (v1);
        \draw[blue] (v5) -- (v2);
        \node at (0,-2.6) {\small $K_5$: no monochromatic $K_3$};
    \end{scope}

    % RIGHT: K_6, with a forced monochromatic triangle highlighted
    \begin{scope}[xshift=3.5cm]
        \foreach \i in {1,2,3,4,5,6} {
            \node[vertex] (w\i) at ({90 + 60*(\i-1)}:1.7) {\small $\i$};
        }
        % Sample colouring with a forced red triangle 1-3-5
        \draw[rede] (w1) -- (w3);
        \draw[rede] (w3) -- (w5);
        \draw[rede] (w5) -- (w1);
        \draw[rede] (w1) -- (w2);
        ...
        \draw[blue] (w2) -- (w5);
        \draw[blue] (w3) -- (w6);
        % Highlight red triangle
        \draw[rede, line width=2.5pt, opacity=0.4] (w1) -- (w3) -- (w5) -- cycle;
        \node at (0,-2.6) {\small $K_6$: red $\triangle\,135$ unavoidable};
    \end{scope}
\end{tikzpicture}
\\
\small Figure 202: Witnessing $R(3,3) = 6$. Left: the unique (up to swap) good $K_5$ colouring. Right: every $2$-colouring of $K_6$ forces a monochromatic triangle.
\label{fig:r33}
\end{figure}

\subsubsection{Probabilistic Lower Bounds}

\begin{theorem}[Erd\H{o}s, 1947]\label{thm:erdos-ramsey}
For all $k \ge 3$, $R(k, k) > 2^{k/2}$.
\end{theorem}

\begin{proof}
Color each edge of $K_N$ red or blue independently and uniformly.
For any fixed $k$-subset $S$ of vertices, the probability that
$K_N[S]$ is monochromatic is $2 \cdot 2^{-\binom{k}{2}}$. Let $X$
be the total number of monochromatic $K_k$'s. Then
\[
  \mathbb{E}[X] \;=\; \binom{N}{k} \cdot 2 \cdot 2^{-\binom{k}{2}}
  \;\le\; \frac{N^k}{k!} \cdot 2^{1 - k(k-1)/2}.
\]
For $N = \lfloor 2^{k/2} \rfloor$ and $k \ge 3$, one checks
$\mathbb{E}[X] < 1$, so there is a coloring realizing $X = 0$. Any
such coloring witnesses $R(k, k) > N$.
\end{proof}

\begin{theorem}[Spencer, via Lovász Local Lemma]\label{thm:spencer-lll}
$R(k, k) > (1 + o(1))\dfrac{\sqrt{2}}{e} \cdot k \cdot 2^{k/2}$.
\end{theorem}

\begin{proof}[Proof sketch]
Apply the symmetric Lovász Local Lemma to the events ``the $k$-set
$S$ is monochromatic''. Two such events are dependent only if $S, S'$
share at least two vertices; for each $S$ there are at most
$\binom{k}{2}\binom{N - 2}{k - 2}$ dependent events. Using
$e \cdot p \cdot (d + 1) \le 1$ and optimizing $N$ yields the factor-of-$k$
improvement over the first-moment bound.
\end{proof}

\begin{theorem}[Off-diagonal: Ajtai--Koml\'os--Szemer\'edi / Kim]
$\dfrac{c \cdot k^2}{\log k} \le R(3, k) \le \dfrac{C \cdot k^2}{\log k}$.
\end{theorem}

The matching upper and lower bounds on $R(3, k)$ (Shearer and Kim,
1995) are one of the crowning achievements of Ramsey theory; the
lower bound uses a subtle \emph{semirandom} / random greedy
construction.

\subsubsection{Ramsey Numbers for Graphs}

\begin{definition}
For graphs $H_1, H_2$, $R(H_1, H_2)$ is the least $N$ such that every
red/blue edge-coloring of $K_N$ contains a red copy of $H_1$ or a
blue copy of $H_2$.
\end{definition}

\begin{theorem}[Chv\'atal, 1977]
For every tree $T$ on $t$ vertices and every $s \ge 2$,
\[
  R(T, K_s) \;=\; (s - 1)(t - 1) + 1.
\]
\end{theorem}
...
\begin{proof}
\emph{Upper bound.} Let $N = (s - 1)(t - 1) + 1$. Color the edges of
$K_N$ red/blue. If the blue graph has chromatic number at least $s$,
then it contains $K_s$ by the degeneracy consequence of the
greedy-coloring argument (each tree is $1$-degenerate). Otherwise,
$N$ vertices partition into $s - 1$ color classes of size $\ge 1$, one
of which has $\ge t$ vertices. The red graph restricted to that class
has minimum degree at least $t - 1$ (it is the ``red'' part of
$K_t$'s subgraph on that class, which is all edges---blue class is
an independent set in blue). Being $(t - 1)$-degenerate, it contains
any tree on $t$ vertices.

\emph{Lower bound.} Partition $(s - 1)(t - 1)$ vertices into $s - 1$
blocks of $t - 1$. Color within-block edges red and cross-block
edges blue; the red graph is a disjoint union of $K_{t - 1}$'s (no
tree on $t$ vertices fits), and the blue graph is $(s - 1)$-partite
(no $K_s$).
\end{proof}

\begin{theorem}[Erd\H{o}s--Gallai; Chv\'atal--Harary]
For the path $P_t$ on $t$ vertices, $R(P_t, P_s) = t + \lfloor s/2 \rfloor - 1$
for $t \ge s \ge 2$.
\end{theorem}

\begin{theorem}[Bondy--Erd\H{o}s]
For cycles of equal length, $R(C_n, C_n) = 2n - 1$ if $n$ is odd
($n \ge 5$), and $R(C_n, C_n) = 3n/2 - 1$ if $n$ is even ($n \ge 6$).
\end{theorem}

\begin{theorem}[Chv\'atal--R\"odl--Szemer\'edi--Trotter]
For every graph $H$ of maximum degree $\Delta$, $R(H, H) \le c_\Delta \cdot |V(H)|$.
\end{theorem}

This linear-in-$|V(H)|$ bound for bounded-degree graphs was a
landmark 1983 result proved via the regularity lemma.

\subsubsection{Hypergraph Ramsey Numbers}

\begin{definition}
$R_k(s, t)$ is the least $N$ such that every red/blue coloring of
the $k$-subsets of $\{1, \dots, N\}$ yields a red $s$-set (all
$k$-subsets red) or a blue $t$-set.
\end{definition}

\begin{theorem}[Erd\H{o}s--Rado stepping-up lemma]
$R_{k+1}(2s, 2t) \ge 2^{R_k(s, t) - 1}$.
\end{theorem}

\begin{corollary}
$R_3(k, k) \ge 2^{c k^2}$ and $R_k(s, s) \ge \text{tower}_{k-1}(cs^2)$,
where $\text{tower}_i$ is the iterated exponential.
\end{corollary}

\begin{remark}
Surprisingly, the doubly-exponential lower bound for $R_3(k, k)$ is
conjectured to be tight up to the constant in the exponent; this is
one of the most fundamental open problems in Ramsey theory.
\end{remark}

\subsubsection{Schur's Theorem and van der Waerden}

\begin{theorem}[Schur, 1916]
For every $k \ge 1$ there exists $N$ such that in every $k$-coloring
of $\{1, 2, \dots, N\}$ there is a monochromatic triple $a, b, c$ with
$a + b = c$.
\end{theorem}

\begin{proof}
Set $N = R(3, 3, \dots, 3)$ ($k$ colors). Given a $k$-coloring of
$\{1, \dots, N\}$, build an edge-coloring of $K_N$ by coloring edge
$ij$ (with $i < j$) by the color of $j - i$. A monochromatic triangle
$i < j < \ell$ gives monochromatic numbers $j - i$, $\ell - j$, and
$\ell - i$ with $(j - i) + (\ell - j) = \ell - i$.
\end{proof}

\begin{theorem}[van der Waerden, 1927]
For every $k \ge 2, \ell \ge 2$ there exists $W(k, \ell)$ such that in
every $k$-coloring of $\{1, 2, \dots, W(k, \ell)\}$ there is a
monochromatic arithmetic progression of length $\ell$.
\end{theorem}

\begin{remark}
Shelah gave primitive-recursive bounds on $W(k, \ell)$; Gowers's
work on Szemer\'edi's theorem on arithmetic progressions in dense
sets gave doubly-exponential bounds on $W$. The best known bounds
are still astronomical.
\end{remark}

\subsubsection{Erd\H{o}s--Szekeres: Happy Ending}

\begin{theorem}[Erd\H{o}s--Szekeres, 1935]\label{thm:erdos-szekeres}
Every sequence of more than $(r - 1)(s - 1)$ distinct real numbers
contains an increasing subsequence of length $r$ or a decreasing
subsequence of length $s$.
\end{theorem}

\begin{proof}[Proof 1: labeling]
Let $(a_i)_{i=1}^n$ be the sequence with $n > (r - 1)(s - 1)$. For
each $i$, let $x_i$ be the length of the longest increasing
subsequence ending at $i$, and $y_i$ the longest decreasing.
If $i < j$ and $a_i < a_j$ then $x_i < x_j$; if $a_i > a_j$ then
$y_i < y_j$. So $(x_i, y_i)$ is injective and lies in
$\{1, \dots, r - 1\} \times \{1, \dots, s - 1\}$ would give $n \le (r - 1)(s - 1)$,
a contradiction.
\end{proof}

\begin{proof}[Proof 2: Ramsey]
Apply $R(r, s) \le \binom{r + s - 2}{r - 1}$ to the graph with vertex
set $\{1, \dots, n\}$ and edges $ij$ colored red if $a_i < a_j$ and
blue otherwise.
\end{proof}

\begin{theorem}[Erd\H{o}s--Szekeres on points in general position]
For every $n \ge 3$ there exists $\mathrm{ES}(n)$ such that any
$\mathrm{ES}(n)$ points in the plane in general position contain $n$
in convex position.
\end{theorem}

The asymptotic $\mathrm{ES}(n) = 2^{n(1 + o(1))}$ was established by
Suk in 2017, tightening a $75$-year-old bound.

\subsubsection{Canonical Ramsey and Induced Ramsey}

\begin{theorem}[Erd\H{o}s--Rado canonical Ramsey, 1950]
For every $k$ there is $N$ such that in any coloring (with arbitrarily
many colors) of the edges of $K_N$, there is a $k$-subset $S$ such
that the coloring of $K_N[S]$ falls into one of four types:
monochromatic, rainbow (all distinct), \emph{min-colored} (color
determined by $\min\{i, j\}$), or \emph{max-colored}.
\end{theorem}

\begin{theorem}[Induced Ramsey; Deuber, Erd\H{o}s--Hajnal--P\'osa]
For every graph $H$ there is a graph $G$ such that in every red/blue
edge-coloring of $G$, there is an induced copy of $H$ in $G$ whose
edges are monochromatic.
\end{theorem}
...
\begin{exercise}
Prove that $R(3, 3) = 6$ by exhibiting a two-coloring of $K_5$ with
no monochromatic triangle (Figure~\ref{fig:r33}, left) and arguing
$R(3, 3) \le 6$ directly via pigeonhole on a vertex.
\end{exercise}

\begin{exercise}
Compute $R(3, 4)$ exactly. (Upper bound: $R(3, 4) \le R(2, 4) + R(3, 3) = 4 + 6 = 10$;
then sharpen to $9$.)
\end{exercise}

\begin{exercise}
Verify the values $R(2, t) = t$ and $R(s, 2) = s$ that anchor the
inductive proof of Theorem~\ref{thm:ramsey}.
\end{exercise}

\paragraph{Medium Questions.}

\begin{exercise}
Prove that $R(s, t) + R(s, t - 1)$ cannot both be even, and use
this to sharpen Theorem~\ref{thm:ramsey} to $R(s, t) < R(s - 1, t) + R(s, t - 1)$
when both addends are even.
\end{exercise}

\begin{exercise}
Prove Schur's theorem for three colors by showing $R(3, 3, 3) \le 17$.
(Due to Greenwood and Gleason; the exact value $R(3, 3, 3) = 17$
requires the Kalbfleisch $K_{16}$ coloring.)
\end{exercise}

\begin{exercise}
Deduce the infinite Ramsey theorem---every $2$-coloring of the edges
of $K_\infty$ has an infinite monochromatic clique---from the finite
version by a compactness argument.
\end{exercise}

\begin{exercise}
Let $\chi(G)$ denote chromatic number. Show that
$R(K_s, G) \le (s - 1)(\chi(G) - 1) + 1$ when $G$ is connected.
\end{exercise}

\paragraph{Hard Questions.}

\begin{exercise}
Use the probabilistic method to show $R(s, t) > (1 - o(1))\, t^{(s-1)/2}$
for fixed $s$ and $t \to \infty$.
\end{exercise}

\begin{exercise}
Prove Chv\'atal's theorem $R(T_n, K_s) = (s - 1)(n - 1) + 1$ for trees,
filling in the degeneracy details.
\end{exercise}

\begin{exercise}
Use the Lov\'asz Local Lemma to show $R(3, k) \ge c \cdot k^2 / \log^2 k$.
(A weaker form of Kim's bound, but significantly sharper than the
first-moment lower bound.)
\end{exercise}

\begin{exercise}
Prove van der Waerden's theorem $W(2, 3) = 9$: any $2$-coloring of
$\{1, \dots, 9\}$ has a monochromatic $3$-AP, and some $2$-coloring
of $\{1, \dots, 8\}$ avoids one.
\end{exercise}

\paragraph{Diagram Questions.}

\begin{exercise}
Reproduce the Greenwood--Gleason $K_{16}$ Kalbfleisch coloring as a
$3$-coloring of $K_{16}$ with no monochromatic triangle. Hint: use
the Paley graph on $\mathbb F_{16}$ for one color class and partition
the remaining edges via additive characters.
\end{exercise}

\begin{exercise}
Draw the bipartite blow-up coloring witnessing the Chv\'atal lower
bound $R(T, K_s) > (s - 1)(t - 1)$: $s - 1$ red cliques of size
$t - 1$ joined blue across blocks. Verify directly that no red
$T_t$ and no blue $K_s$ appears.
\end{exercise}

\subsection{The Probabilistic Method}

The \emph{probabilistic method}, pioneered by Erd\H{o}s in the 1940s
and 50s, is now the single most pervasive technique in combinatorics.
The philosophy: to prove an object with property $P$ exists, sample a
random object from an appropriate distribution and show
$\Pr[\text{sample has } P] > 0$. In practice this unfolds as four
refinable tools: \emph{first moment} (expectation and Markov),
\emph{alteration} (modify the random object to fix defects),
\emph{second moment} (Chebyshev for sharp thresholds), and
\emph{Lov\'asz Local Lemma} (escape dependencies). We then discuss
\emph{concentration} via martingales (Azuma), entropy, and
\emph{dependent random choice}.

\subsubsection{First Moment: Linearity of Expectation}
...
\begin{lemma}[First moment principle]
If $X$ is a non-negative integer-valued random variable with
$\mathbb{E}[X] < 1$ then $\Pr[X = 0] > 0$.
\end{lemma}

\begin{theorem}[Large independent set]\label{thm:largeind}
Every graph $G$ with $n$ vertices and average degree $d \ge 1$ has an
independent set of size at least $n/(d + 1)$.
\end{theorem}

\begin{proof}
Pick a uniformly random permutation $\pi$ of $V(G)$, and let
$S = \{\, v : \pi(v) < \pi(w) \text{ for every } w \in N(v) \,\}$.
Then $S$ is always an independent set: if $uv \in E$ and both
$u, v \in S$, both would precede the other in $\pi$, absurd.

Each $v$ joins $S$ iff $v$ is the first among the $d(v) + 1$ vertices
in $\{v\} \cup N(v)$, so $\Pr[v \in S] = 1/(d(v) + 1)$. By linearity
of expectation and Jensen's inequality,
\[
  \mathbb{E}[|S|] = \sum_{v \in V} \frac{1}{d(v) + 1}
  \;\ge\; \frac{n}{\frac{1}{n}\sum_v d(v) + 1} = \frac{n}{d + 1}.
\]
So some realization has $|S| \ge n/(d + 1)$.
\end{proof}

\begin{corollary}[Tur\'an-type lower bound for independence]
$\alpha(G) \ge \sum_v 1/(d(v) + 1)$.
\end{corollary}

\begin{theorem}[Hamiltonian paths in tournaments]
The expected number of Hamiltonian paths in a tournament on $n$
vertices (edges oriented uniformly at random) is $n!/2^{n - 1}$. In
particular every tournament has at least one Hamiltonian path.
\end{theorem}

\begin{proof}
For a fixed ordering $v_1, \dots, v_n$ of vertices, the probability
that all $n - 1$ consecutive arcs point forward is $2^{-(n - 1)}$.
Sum over all $n!$ orderings: expectation equals $n!/2^{n - 1}$.
\end{proof}

\begin{theorem}[Dominating set; Alon]
Every graph $G$ with minimum degree $\delta \ge 1$ has a dominating
set of size at most $n(1 + \ln(\delta + 1))/(\delta + 1)$.
\end{theorem}

\begin{proof}
Let $p = \ln(\delta + 1)/(\delta + 1)$. Include each vertex in $S$
independently with probability $p$. Let $T = \{\, v \notin S : N(v) \cap S = \emptyset \,\}$.
Then $S \cup T$ is dominating, and
\[
  \mathbb{E}[|S| + |T|]
  \le np + \sum_v (1 - p)^{d(v) + 1}
  \le np + n(1 - p)^{\delta + 1}
  \le np + n e^{-p(\delta + 1)}.
\]
Plugging in $p$ yields the bound.
\end{proof}

\subsubsection{The Alteration (Deletion) Method}

When the random object has a few more ``bad'' features than desired,
simply delete them.

\begin{theorem}[Erd\H{o}s lower bound for Ramsey, via deletion]
$R(k, k) > (1 + o(1)) \dfrac{k}{e} \cdot 2^{k/2}$.
\end{theorem}

\begin{proof}
Color edges of $K_n$ uniformly. Expected number of monochromatic
$k$-cliques $X = 2\binom{n}{k} 2^{-\binom{k}{2}}$. Delete one vertex
from each; the remainder witnesses $R(k, k) > n - X$. Optimize over
$n$ to obtain the bound, improving Theorem~\ref{thm:erdos-ramsey} by
a factor $2/e$.
\end{proof}

\begin{theorem}[High girth, high chromatic number; Erd\H{o}s, 1959]\label{thm:girthchromatic}
For every $k, g \ge 3$ there is a graph $G$ with girth $g(G) \ge g$
and chromatic number $\chi(G) \ge k$.
\end{theorem}

\begin{proof}
Fix $\theta \in (0, 1/g)$ and let $p = n^{\theta - 1}$. Sample
$G \sim G(n, p)$. The expected number of cycles of length less than
$g$ is
\[
  \sum_{\ell = 3}^{g - 1} \frac{n!}{(n - \ell)! \cdot 2\ell} p^\ell
  \;\le\; \sum_{\ell = 3}^{g - 1} (np)^\ell = o(n),
\]
so by Markov with probability $\ge 1/2$ the number of short cycles is
$\le n/2$.

Meanwhile, the probability that $G$ has an independent set of size
$\lceil n/k \rceil$ is at most
\[
  \binom{n}{n/k} (1 - p)^{\binom{n/k}{2}} \le 2^n e^{-p n^2/(3 k^2)} \to 0.
\]
So with positive probability both events hold: $G$ has fewer than
$n/2$ short cycles and $\alpha(G) < n/k$. Delete one vertex from each
short cycle to obtain $G'$ with $|V(G')| \ge n/2$, girth $\ge g$, and
$\alpha(G') < n/k \le 2|V(G')|/k$. Thus $\chi(G') \ge |V(G')|/\alpha(G') > k/2$;
rescale.
\end{proof}

\begin{remark}
No constructive proof of Theorem~\ref{thm:girthchromatic} with
comparable parameters was known until the expander-graph constructions
of Lubotzky--Phillips--Sarnak in the 1980s.
\end{remark}

\subsubsection{Second Moment and Paley--Zygmund}

\begin{theorem}[Chebyshev]\label{thm:chebyshev}
For any random variable $X$ and $t > 0$,
$\Pr[|X - \mathbb{E}[X]| \ge t] \le \mathrm{Var}(X)/t^2$.
\end{theorem}

\begin{lemma}[Paley--Zygmund]
For non-negative $X$ with $\mathbb{E}[X] > 0$,
\[
  \Pr[X > 0] \;\ge\; \frac{(\mathbb{E}[X])^2}{\mathbb{E}[X^2]}
  \;=\; \frac{1}{1 + \mathrm{Var}(X)/(\mathbb{E}[X])^2}.
\]
\end{lemma}

\begin{theorem}[Triangle threshold in $G(n, p)$]\label{thm:trianglethresh}
As $n \to \infty$, $G(n, p)$ contains a triangle with probability
tending to $0$ if $pn \to 0$, and to $1$ if $pn \to \infty$.
\end{theorem}

\begin{proof}
Let $X$ be the number of triangles. Then $\mathbb{E}[X] = \binom{n}{3} p^3$,
so Markov gives $\Pr[X \ge 1] \to 0$ when $np \to 0$. For the other
direction, $\mathrm{Var}(X) = O(n^4 p^5)$ and
$(\mathbb{E} X)^2 = \Theta(n^6 p^6)$, so the Paley--Zygmund ratio
tends to $1$ when $np \to \infty$.
\end{proof}

\subsubsection{The Lov\'asz Local Lemma}

First-moment and alteration arguments fail when dependencies are
dense; LLL escapes them.

\begin{definition}[Dependency graph]
Given events $A_1, \dots, A_m$, a \textbf{dependency graph} $D$ has
vertex set $[m]$ with $A_i$ mutually independent of
$\{A_j : j \notin N_D(i) \cup \{i\}\}$.
\end{definition}

\begin{theorem}[General LLL; Erd\H{o}s--Lov\'asz]\label{thm:lll-general}
If there exist $x_1, \dots, x_m \in (0, 1)$ with
$\Pr[A_i] \le x_i \prod_{j \in N_D(i)}(1 - x_j)$, then
\[
  \Pr\Big[\bigcap_i \overline{A_i}\Big]
  \;\ge\; \prod_i (1 - x_i) \;>\; 0.
\]
\end{theorem}

\begin{theorem}[Symmetric LLL]\label{thm:lll}
If $\Pr[A_i] \le p$ for all $i$, each $A_i$ is mutually independent
of all but at most $d$ other events, and $e p (d + 1) \le 1$, then
$\Pr[\bigcap_i \overline{A_i}] > 0$.
\end{theorem}

\begin{proof}[Proof sketch]
Set $x_i = 1/(d + 1)$ in Theorem~\ref{thm:lll-general} and verify
$p \le 1/(e(d + 1))$ using $(1 - 1/(d + 1))^d \ge 1/e$.
\end{proof}

\begin{theorem}[LLL lower bound for $R(k, k)$]
$R(k, k) > (1 + o(1)) \dfrac{\sqrt{2}}{e} k \cdot 2^{k/2}$.
\end{theorem}

\begin{proof}
Color edges of $K_n$ uniformly. For each $k$-set $S$ let $A_S$ be
``$K_n[S]$ monochromatic''; $p = 2^{1 - \binom{k}{2}}$. Events $A_S$
and $A_{S'}$ are independent unless $|S \cap S'| \ge 2$, so the
dependency degree is $d \le \binom{k}{2}\binom{n - 2}{k - 2}$. Apply
the symmetric LLL and optimize $n$.
\end{proof}

\begin{theorem}[Moser--Tardos, 2010]\label{thm:mosertardos}
Under the symmetric LLL hypothesis with events defined by independent
variables, the following algorithm terminates in expected polynomial
time with an assignment avoiding every $A_i$:

\emph{Sample all variables; while some $A_i$ holds, pick one and
resample its variables.}
\end{theorem}

\begin{remark}
Moser--Tardos turns LLL from a pure existence result into a
constructive algorithm. The proof uses \emph{entropy compression}:
a log of resampling events is encoded as a prefix of a fair coin
sequence, so too-long executions contradict the entropy of the
random tape.
\end{remark}

\subsubsection{Concentration: Azuma and Talagrand}

\begin{definition}[Martingale, bounded differences]
A sequence $(X_0, \dots, X_n)$ is a \textbf{martingale} with respect
to $(\mathcal F_i)$ if $\mathbb{E}[X_{i + 1} \mid \mathcal F_i] = X_i$.
It has \textbf{bounded differences} $c_i$ if $|X_i - X_{i - 1}| \le c_i$
a.s.
\end{definition}

\begin{theorem}[Azuma--Hoeffding]\label{thm:azuma}
For a martingale with $|X_i - X_{i - 1}| \le c_i$,
\[
  \Pr[|X_n - X_0| \ge t]
  \;\le\; 2 \exp\!\Big(-\frac{t^2}{2 \sum_i c_i^2}\Big).
\]
\end{theorem}

\begin{corollary}[Lipschitz concentration in $G(n, p)$]
Any $f$ on $G(n, p)$ with $|f(G) - f(G')| \le 1$ when $G, G'$ differ
on one edge satisfies
\[
  \Pr[|f - \mathbb{E} f| \ge t] \le 2\exp(-t^2/(2\binom{n}{2})).
\]
\end{corollary}

\begin{theorem}[Talagrand's inequality, informal]
Let $f : \{0, 1\}^n \to \mathbb{R}$ be $1$-Lipschitz and $c$-certifiable,
meaning $f(x) \ge s$ is certified by a subset $I \subseteq [n]$ of
size $\le cs$. Let $X = f(\boldsymbol{\xi})$ for independent coordinates
$\boldsymbol{\xi}$ with median $M$. Then
\[
  \Pr[X \le M - t] \le 2\exp(-t^2/(4cM)),
  \qquad
  \Pr[X \ge M + t] \le 2\exp(-t^2/(4c(M + t))).
\]
\end{theorem}

Talagrand's inequality is the preferred concentration tool for
triangle counts and $\chi(G(n, p))$ because the certificate condition
is often much sharper than the Lipschitz constant alone would suggest.

\subsubsection{Dependent Random Choice}

\begin{lemma}[Dependent random choice]\label{lem:drc}
Let $G$ be a graph on $n$ vertices with average degree $d$. For
integers $a, r, m, t \ge 1$, if
$d^t/n^{t - 1} - \binom{n}{r}(m/n)^t \ge a$,
then $G$ has a subset $U$ with $|U| \ge a$ in which every $r$-tuple
has at least $m$ common neighbors.
\end{lemma}

\begin{proof}[Proof sketch]
Let $T$ be a random $t$-set and $U = \bigcap_{v \in T} N(v)$. Then
$\mathbb{E}[|U|] \ge d^t/n^{t - 1}$. Expected number of bad $r$-sets
inside $U$ is at most $\binom{n}{r}(m/n)^t$. Delete and apply
linearity.
\end{proof}

\begin{corollary}[Bipartite Tur\'an via DRC]
Every graph with at least $n^{2 - 1/t}$ edges contains $K_{t, t}$.
\end{corollary}

Dependent random choice powers modern proofs of Ramsey-type
statements for sparse graphs, the Balog--Szemer\'edi--Gowers theorem,
and refined bipartite Tur\'an bounds.

\subsubsection{The Entropy Method}

\begin{definition}[Shannon entropy]
For a discrete random variable $X$,
$H(X) = -\sum_x \Pr[X = x] \log \Pr[X = x]$. For jointly distributed
$(X, Y)$, the conditional entropy is
$H(X \mid Y) = \mathbb E_y H(X \mid Y = y)$.
\end{definition}

\begin{theorem}[Shearer's lemma]
If $\mathcal F$ is a collection of subsets of $[n]$ with each element
covered at least $k$ times, then for any joint distribution on
$(X_1, \dots, X_n)$,
\[
  k \cdot H(X_1, \dots, X_n) \;\le\; \sum_{F \in \mathcal F} H(X_F).
\]
\end{theorem}

\begin{corollary}[Kruskal--Katona-type triangle bound]
A graph with $m$ edges has at most $\tfrac{1}{6}(2m)^{3/2}$ triangles.
\end{corollary}
...
\begin{exercise}
Using linearity of expectation, show that every graph on $m$ edges
has a bipartite subgraph with at least $m/2$ edges, and a $k$-partite
subgraph with at least $(1 - 1/k)m$ edges.
\end{exercise}

\begin{exercise}
Show that every tournament on $n$ vertices has a dominating set of
size at most $\lceil \log_2 n \rceil$. (Sample uniformly random
$k$-sets and union-bound.)
\end{exercise}

\begin{exercise}
Compute the variance of the triangle count $X$ in $G(n, p)$ for
$p = 1/2$ and verify $\mathrm{Var}(X) = O(n^4)$ while
$\mathbb{E}[X] = \Theta(n^3)$.
\end{exercise}

\paragraph{Medium Questions.}

\begin{exercise}
Use the vertex-exposure martingale and Azuma's inequality to show
that $\chi(G(n, 1/2))$ is concentrated within $O(\sqrt{n \log n})$ of
its median.
\end{exercise}

\begin{exercise}
Prove that every tournament contains at least $n!/2^{n - 1}$
Hamiltonian paths (Szele). Use this to give a lower bound on the
permanent of the all-$1/2$ matrix of size $n$.
\end{exercise}

\begin{exercise}
Identify the regime $p = p(n)$ where the triangle count $X$ in
$G(n,p)$ satisfies $X/\mathbb{E}[X] \to 1$ in probability via the
...
\begin{exercise}
Use LLL to show that every graph $G$ of maximum degree $\Delta$ can
be edge-coloured by at most $C \cdot \Delta$ colours such that each
colour class is \emph{acyclic}. (This is Alon's acyclic chromatic
theorem; $C = 64$ suffices.)
\end{exercise}

\begin{exercise}
State and prove the \emph{Lopsided} LLL: the dependency condition may
be relaxed to a bound on $\Pr[A_i \mid \bigcap_{j \in J} \overline{A_j}]$
for $J$ disjoint from the dependency neighborhood of $i$.
\end{exercise}

\begin{exercise}
Use dependent random choice (Lemma~\ref{lem:drc}) to prove:
every $n$-vertex graph with $\ge n^{2 - 1/t}$ edges contains $K_{t, t}$.
\end{exercise}

\begin{exercise}
Use Shearer's entropy lemma to bound the number of labeled copies
of $C_4$ in a graph with $m$ edges.
\end{exercise}

\begin{exercise}
Prove the second-moment concentration for subgraph counts: if
$H$ has $v$ vertices and $e$ edges and $np^{e/v} \to \infty$, the
number of copies of $H$ in $G(n, p)$ concentrates around its mean.
\end{exercise}

\paragraph{Diagram Questions.}

\begin{exercise}
Draw the dependency graph $D$ for the events ``the $4$-set $S$ is
monochromatic'' as $S$ ranges over $\binom{[6]}{4}$ in the
random $2$-coloring of $E(K_6)$. Estimate the dependency degree $d$
and check whether the symmetric LLL applies.
\end{exercise}

\begin{exercise}
For a triangle-free graph $G$ found by the deletion method on
$G(n, c/n)$ with $c = 5$, sketch (schematically) the typical local
structure: degree distribution, isolated vertices, short cycles,
and the surviving sparse independent-set witness for high
chromatic number.
\end{exercise}
...
matrices associated with a graph---primarily the adjacency matrix
$A$ and the Laplacian $L$. The central thesis is that these spectra
encode deep combinatorial information: connectivity, expansion,
bipartiteness, walk-counting, mixing times of random walks, and the
asymptotic complexity of many combinatorial algorithms.

\subsubsection{Matrices of a Graph}

\begin{definition}[Adjacency and Laplacian matrices]
Let $G = (V, E)$ be a graph on $V = \{1, \dots, n\}$.
\begin{itemize}
  \item The \textbf{adjacency matrix} $A \in \{0, 1\}^{n \times n}$
        has $A_{ij} = 1$ iff $ij \in E$.
  \item The \textbf{degree matrix} $D$ is diagonal with $D_{ii} = d(i)$.
  \item The \textbf{combinatorial Laplacian} is $L = D - A$.
  \item The \textbf{normalized Laplacian} is
        $\mathcal L = I - D^{-1/2} A D^{-1/2}$ (where degrees are
        positive).
  \item The \textbf{signless Laplacian} is $Q = D + A$.
\end{itemize}
All these matrices are symmetric (when $G$ is undirected), hence have
real spectra and an orthonormal eigenbasis.
\end{definition}

\begin{definition}[Spectrum notation]
For the adjacency matrix we write
$\lambda_1 \ge \lambda_2 \ge \cdots \ge \lambda_n$. For the Laplacian,
$0 = \mu_1 \le \mu_2 \le \cdots \le \mu_n$.
\end{definition}

\begin{theorem}[Spectral theorem]
Any real symmetric $n \times n$ matrix $M$ admits an orthonormal
eigenbasis $v_1, \dots, v_n$ with real eigenvalues $\tau_1, \dots, \tau_n$;
equivalently $M = \sum_i \tau_i v_i v_i^\top$.
\end{theorem}

\subsubsection{Walk Counting and Basic Identities}

\begin{proposition}[Walk counting]\label{prop:walkcount}
For any $k \ge 0$, $(A^k)_{ij}$ equals the number of walks of length
$k$ from $i$ to $j$.
\end{proposition}

\begin{corollary}[Traces]
$\mathrm{tr}(A^2) = 2|E|$, $\mathrm{tr}(A^3) = 6 \cdot (\# \text{triangles in } G)$.
\end{corollary}

\begin{proposition}[Laplacian quadratic form]\label{prop:lapquadratic}
For any $x \in \mathbb R^n$,
\[
  x^\top L x \;=\; \sum_{ij \in E} (x_i - x_j)^2 \;\ge\; 0.
\]
Hence $L$ is positive semidefinite, all $\mu_i \ge 0$, and
$\mathbf 1$ is always a $0$-eigenvector.
\end{proposition}

\begin{theorem}[Multiplicity of $0$]\label{thm:laplacian-components}
The multiplicity of the eigenvalue $0$ of $L$ equals the number of
connected components of $G$.
\end{theorem}

\begin{proof}
If $x^\top L x = 0$ then $x$ is constant on every connected component
by Proposition~\ref{prop:lapquadratic}. Conversely, the indicator
vectors of the components form a $0$-eigenspace basis.
\end{proof}

\begin{proposition}[Regular case]
If $G$ is $d$-regular, then $L = dI - A$, so $\mu_i = d - \lambda_{n + 1 - i}$.
In particular $\lambda_1 = d$ and $\lambda_n \ge -d$.
\end{proposition}

\begin{theorem}[Bipartiteness and $\lambda_n = -d$]
A $d$-regular graph $G$ is bipartite if and only if $\lambda_n = -d$.
The multiplicity of $-d$ equals the number of bipartite connected
components.
\end{theorem}

\subsubsection{The Min-Max Principle and Rayleigh Quotient}

\begin{definition}[Rayleigh quotient]
For a symmetric matrix $M$ and $x \ne 0$, the \textbf{Rayleigh
quotient} is $R_M(x) = x^\top M x / x^\top x$.
\end{definition}

\begin{theorem}[Courant--Fischer]\label{thm:courantfischer}
Let $M$ be symmetric with eigenvalues $\tau_1 \ge \cdots \ge \tau_n$.
Then
\[
  \tau_k \;=\; \max_{\dim U = k} \min_{x \in U, x \ne 0} R_M(x)
          \;=\; \min_{\dim U = n - k + 1} \max_{x \in U, x \ne 0} R_M(x).
\]
\end{theorem}

\begin{corollary}[Interlacing]
Let $M$ be a symmetric $n \times n$ matrix and $M'$ any principal
submatrix of order $m$. Then the eigenvalues
$\tau_1' \ge \cdots \ge \tau_m'$ of $M'$ interlace those of $M$:
$\tau_{i + n - m}(M) \le \tau_i'(M') \le \tau_i(M)$.
\end{corollary}

\begin{corollary}[Cauchy interlacing for graphs]
If $H$ is an induced subgraph of $G$, the adjacency eigenvalues of
$H$ interlace those of $G$.
\end{corollary}

\subsubsection{Perron--Frobenius}

\begin{theorem}[Perron--Frobenius]\label{thm:perron}
Let $M \ge 0$ be a non-negative irreducible matrix. Then:
\begin{enumerate}
  \item The spectral radius $\rho(M)$ is a simple eigenvalue.
  \item There exists a strictly positive eigenvector $v > 0$ for
        $\rho(M)$.
  \item $\rho(M) \ge |\tau|$ for every eigenvalue $\tau$, with strict
        inequality unless the period of $M$'s underlying digraph
        divides $\arg(\tau/\rho)$.
\end{enumerate}
\end{theorem}

\begin{corollary}
For a connected graph $G$, $\lambda_1(A) = \rho(A)$ is simple and its
eigenvector has strictly positive entries.
\end{corollary}

\begin{corollary}
$\lambda_1(A(G)) \ge \bar d = 2|E|/n$ (average degree), with equality
iff $G$ is regular.
\end{corollary}

\begin{proof}
Take $x = \mathbf 1/\sqrt n$. Then $R_A(\mathbf 1) = \bar d \le \lambda_1$
by Courant--Fischer.
\end{proof}

\subsubsection{The Matrix-Tree Theorem}

\begin{theorem}[Kirchhoff, 1847]\label{thm:matrixtree}
Let $G$ be a connected graph. The number of spanning trees of $G$
equals any cofactor of $L$: delete any row $i$ and column $i$, and
take the determinant of the resulting $(n - 1) \times (n - 1)$ matrix.
Equivalently,
\[
  \tau(G) \;=\; \frac{1}{n} \mu_2 \mu_3 \cdots \mu_n.
\]
\end{theorem}

\begin{proof}[Proof sketch via Cauchy--Binet]
Orient the edges of $G$ arbitrarily and define the \emph{signed
incidence matrix} $B \in \{-1, 0, 1\}^{n \times m}$ with
$B_{ve} = \pm 1$ depending on whether $v$ is the tail or head of $e$.
Then $L = B B^\top$. The reduced incidence matrix $\tilde B$
(obtained by deleting row $i$) has $\det(\tilde B \tilde B^\top)$
equal by Cauchy--Binet to the sum over $(n - 1)$-subsets $S$ of
edges of $(\det \tilde B_S)^2$. But $\det \tilde B_S = \pm 1$ if $S$
is a spanning tree, $0$ otherwise; so the cofactor is the spanning
tree count.
\end{proof}
...
\begin{proof}
$L(K_n) = n I - J$ (where $J$ is all-ones). Its eigenvalues are $0$
(once) and $n$ ($n - 1$ times), so
$\tau(K_n) = \frac{1}{n} \cdot n^{n - 1} = n^{n - 2}$.
\end{proof}

\subsubsection{Algebraic Connectivity and Cheeger's Inequality}

\begin{definition}[Algebraic connectivity and Cheeger constant]
The \textbf{algebraic connectivity} of $G$ is $\mu_2$. The
\textbf{edge expansion} (or \emph{Cheeger constant}) is
\[
...
\begin{theorem}[Discrete Cheeger inequality]\label{thm:cheeger}
For a $d$-regular graph $G$,
\[
  \frac{\mu_2}{2} \;\le\; h(G) \;\le\; \sqrt{2 d \mu_2}.
\]
\end{theorem}

\begin{proof}[Proof of the easy direction]
Let $S$ be the minimizer and let $\mathbf 1_S$ be its indicator. The
vector $x = \mathbf 1_S - \frac{|S|}{n}\mathbf 1$ is orthogonal to
$\mathbf 1$, so by Courant--Fischer
\[
  \mu_2 \;\le\; \frac{x^\top L x}{x^\top x}
       \;=\; \frac{|E(S, V \setminus S)|}{|S|(1 - |S|/n)}
       \;\le\; \frac{2 |E(S, V \setminus S)|}{|S|}
       \;=\; 2 h(G).
\]
\end{proof}

\begin{proof}[Proof of the hard direction, sketch]
Let $v_2$ be the Fiedler eigenvector of $L$. Sort the vertices by
$v_2$-coordinate and consider the $n - 1$ \emph{threshold cuts}
$S_k = \{v_{(1)}, \dots, v_{(k)}\}$. Show that the minimum Cheeger
ratio over all $S_k$ is at most $\sqrt{2 d \mu_2}$. This is the
spectral analogue of the Federer--Fleming isoperimetric trick.
\end{proof}

\subsubsection{Expander Graphs}

\begin{definition}[Expander]
An infinite family $\{G_n\}$ of $d$-regular graphs is a
\textbf{(one-sided) spectral expander} if $\max\{\lambda_2(G_n), -\lambda_n(G_n)\} \le \lambda$
for a fixed $\lambda < d$. It is a \textbf{combinatorial expander} if
$h(G_n) \ge c > 0$. By Cheeger (Theorem~\ref{thm:cheeger}), the two
notions are equivalent up to quantitative parameters.
\end{definition}

\begin{theorem}[Expander Mixing Lemma]\label{thm:emixing}
Let $G$ be $d$-regular on $n$ vertices with
$|\lambda| = \max(|\lambda_2|, |\lambda_n|)$. For all $S, T \subseteq V$,
\[
  \Big|\, |E(S, T)| - \frac{d |S| |T|}{n} \,\Big|
  \;\le\; |\lambda|\sqrt{|S| |T|}.
\]
\end{theorem}

\begin{proof}
Write $\mathbf 1_S = \alpha \mathbf 1 + x$ and $\mathbf 1_T = \beta \mathbf 1 + y$
with $\alpha = |S|/n, \beta = |T|/n$ and $x, y \perp \mathbf 1$. Then
\[
  \mathbf 1_S^\top A \mathbf 1_T = |E(S, T)| = \alpha \beta \cdot n d + x^\top A y.
\]
The second term is bounded by $|\lambda| \|x\|\|y\| \le |\lambda| \sqrt{|S||T|}$.
\end{proof}

\begin{theorem}[Alon--Boppana, 1986]
For every $d$-regular graph on $n$ vertices,
\[
  \lambda_2(G) \;\ge\; 2\sqrt{d - 1} - o_n(1).
\]
\end{theorem}

\begin{definition}[Ramanujan graph]\label{def:ramanujan}
A $d$-regular graph $G$ is \textbf{Ramanujan} if
$\max(\lambda_2, -\lambda_n) \le 2\sqrt{d - 1}$.
\end{definition}

\begin{theorem}[Lubotzky--Phillips--Sarnak; Margulis, 1988]
For every prime $p \equiv 1 \pmod 4$, there is an explicit infinite
family of $(p + 1)$-regular Ramanujan graphs.
\end{theorem}

\begin{theorem}[Marcus--Spielman--Srivastava, 2015]
For every $d \ge 3$ and every $n$ divisible by some small integer,
there is a $d$-regular bipartite Ramanujan graph on $n$ vertices.
\end{theorem}

Marcus--Spielman--Srivastava introduced the \emph{interlacing
polynomials} method to solve the Kadison--Singer problem; the
Ramanujan construction was a byproduct.

\subsubsection{Random Walks and Mixing}

\begin{definition}[Random walk matrix]
The \textbf{transition matrix} of the simple random walk on $G$ is
$P = D^{-1} A$. For a $d$-regular graph, $P = A/d$ and its spectrum
is $\{\lambda_i/d\}$.
\end{definition}

\begin{theorem}[Mixing time bound]\label{thm:mixing}
For a connected non-bipartite $d$-regular graph $G$, the simple
random walk has total variation mixing time
\[
  t_{\mathrm{mix}}(\varepsilon) \;\le\; \frac{\log(n/\varepsilon)}{1 - \lambda_2/d}.
\]
\end{theorem}

\begin{proof}[Proof sketch]
The stationary distribution is uniform. The second-largest absolute
eigenvalue governs the spectral gap; by the spectral decomposition
$P^t$ converges to $\mathbf 1 \mathbf 1^\top/n$ at rate $(\lambda_2/d)^t$.
The $\log(n/\varepsilon)$ factor absorbs the $L^2 \to L^1$ conversion.
\end{proof}

\subsubsection{Hoffman's Bound and Chromatic Number}

\begin{theorem}[Hoffman's bound]\label{thm:hoffman}
For any graph $G$,
\[
  \chi(G) \;\ge\; 1 - \frac{\lambda_1(A)}{\lambda_n(A)}.
\]
\end{theorem}

\begin{proof}[Proof sketch]
Suppose $G$ admits a proper $k$-coloring with color classes $C_1, \dots, C_k$.
Let $y = (y_1, \dots, y_k)$ optimize the quadratic form
$y^\top B y / y^\top y$ for $B$ a $k \times k$ auxiliary matrix
built from block-averaged eigencomponents. An interlacing argument
applied to $A$ restricted to a color class (an independent set) gives
the bound.
\end{proof}

\begin{theorem}[Independence number bound]
For a regular graph,
\[
  \alpha(G) \;\le\; \frac{-\lambda_n}{\lambda_1 - \lambda_n} \cdot n.
\]
\end{theorem}
...
A graph $G$ on $n$ vertices is \textbf{strongly regular} with
parameters $(n, k, \lambda, \mu)$ if it is $k$-regular, every pair
of adjacent vertices has $\lambda$ common neighbors, and every pair
of non-adjacent vertices has $\mu$ common neighbors.
\end{definition}

\begin{theorem}[Spectrum of SRG]\label{thm:srgspectrum}
A connected strongly regular graph with parameters $(n, k, \lambda, \mu)$
has exactly three adjacency eigenvalues: $k$ with multiplicity $1$, and
\[
  r, s = \tfrac{1}{2}\Big((\lambda - \mu) \pm \sqrt{(\lambda - \mu)^2 + 4(k - \mu)}\Big),
\]
with multiplicities determined by $k, r, s, n$.
\end{theorem}

Strongly regular graphs are a central playground for algebraic graph
theory, connecting to finite geometries (projective planes,
generalized quadrangles), coding theory, and the classification of
finite simple groups.

\begin{example}[Petersen graph spectrum]
The Petersen graph $P$ is the strongly regular graph with parameters
$(10, 3, 0, 1)$: $3$-regular, no triangles, and any two non-adjacent
vertices share exactly one common neighbor. By
Theorem~\ref{thm:srgspectrum} its adjacency spectrum is
$\{3^{(1)}, 1^{(5)}, (-2)^{(4)}\}$ (eigenvalues with multiplicities).
The Hoffman bound gives $\chi(P) \ge 1 - 3/(-2) = 5/2$, so
$\chi(P) \ge 3$; in fact $\chi(P) = 3$.
...
\begin{tikzpicture}[
    ...
    \foreach \i in {0,1,2,3,4} {
        \node[vertex] (o\i) at ({90 + 72*\i}:2.0) {\small \i};
    }
    % Inner pentagram (vertices 5-9)
    \foreach \i in {0,1,2,3,4} {
        \pgfmathtruncatemacro{\j}{\i+5}
        \node[vertex] (i\i) at ({90 + 72*\i}:1.0) {\small \j};
    }
    % Pentagon edges
    ...
    \draw[edge] (o4) -- (o0);
    % Spokes
    \foreach \i in {0,1,2,3,4} \draw[edge] (o\i) -- (i\i);
    % Pentagram edges (skip 2)
    \foreach \i in {0,1,2,3,4} {
        \pgfmathtruncatemacro{\j}{mod(\i+2,5)}
        \draw[edge] (i\i) -- (i\j);
    }
\end{tikzpicture}
\\
\small Figure 203: The Petersen graph; spectrum $\{3, 1, 1, 1, 1, 1, -2, -2, -2, -2\}$.
\label{fig:petersen-spectrum}
\end{figure}
\end{example}
...
\begin{exercise}
Compute the adjacency spectrum of $K_n$, $K_{n, n}$, $C_n$, and the
Petersen graph.
\end{exercise}

\begin{exercise}
Use the Matrix-Tree theorem to count spanning trees of $K_{m, n}$.
Answer: $m^{n - 1} n^{m - 1}$.
\end{exercise}

\begin{exercise}
Prove that a graph is bipartite iff its adjacency spectrum is
symmetric about $0$.
\end{exercise}
...
\begin{exercise}
For the cycle $C_n$, compute the Laplacian eigenvalues explicitly as
$2 - 2\cos(2\pi k/n)$ for $k = 0, 1, \dots, n - 1$. Identify $\mu_2$
and comment on how it scales with $n$.
\end{exercise}

\begin{exercise}
Prove that the hypercube $Q_n = K_2^{\square n}$ has Laplacian
spectrum $\{2k : k = 0, 1, \dots, n\}$ with multiplicity $\binom{n}{k}$.
\end{exercise}

\begin{exercise}
Prove Hoffman's theorem: $\chi(G) \ge 1 - \lambda_1/\lambda_n$, by
applying interlacing to any independent-set partition.
\end{exercise}

\begin{exercise}
Use the expander mixing lemma to give an alternate proof of
Cheeger's upper bound: $h(G) \le \sqrt{2 d \mu_2}$.
\end{exercise}

\paragraph{Hard Questions.}

\begin{exercise}
Prove Theorem~\ref{thm:srgspectrum} on the three-eigenvalue spectrum
of a strongly regular graph.
\end{exercise}

\begin{exercise}
Show that if $G$ is a $d$-regular $\lambda$-expander, then the
diameter of $G$ satisfies $\mathrm{diam}(G) \le \lceil \log_{d/\lambda} n \rceil$.
\end{exercise}

\begin{exercise}
Prove the Alon--Boppana bound $\lambda_2 \ge 2\sqrt{d - 1} - o(1)$ by
a trace / walk-counting argument on the $d$-regular infinite tree.
\end{exercise}

\paragraph{Diagram Questions.}

\begin{exercise}
Draw the Fiedler eigenvector $v_2$ of the path $P_6$ as a colored
sequence (positive on one end, negative on the other) and use the
threshold-cut sweep on $v_2$ to recover the bisection $\{1, 2, 3\}$
vs $\{4, 5, 6\}$. Verify directly that this is the Cheeger optimal
cut.
\end{exercise}

\begin{exercise}
Draw the Petersen graph (Figure~\ref{fig:petersen-spectrum}) with a
proper $3$-coloring of its vertices, and verify that no smaller
proper coloring exists by combining the Hoffman bound with a parity
argument on the outer pentagon.
\end{exercise}

\subsection{Random Graphs}

Random graph theory, initiated by Erd\H{o}s and R\'enyi in their
landmark 1959--61 papers, studies the typical behavior of graphs
sampled from a distribution. The discovery of \emph{sharp thresholds}
and \emph{phase transitions} was both mathematically startling and
enormously influential: the same phenomena appear in percolation,
spin glasses, random matrices, and the complexity of random
constraint satisfaction.

\subsubsection{Random Graph Models}

\begin{definition}[Erd\H{o}s--R\'enyi model]\label{def:gnp}
$G(n, p)$ is the probability space of graphs on $n$ labeled vertices
in which each of the $\binom{n}{2}$ possible edges appears
independently with probability $p = p(n)$.
\end{definition}

\begin{definition}[Uniform model]
$G(n, M)$ is the uniform distribution over graphs on $n$ labeled
vertices having exactly $M$ edges. The models $G(n, p)$ and
$G(n, M)$ with $M \approx p\binom{n}{2}$ are ``essentially equivalent''
for most purposes via conditioning.
\end{definition}

\begin{definition}[Random regular model]
$G_{n, d}$ is the uniform distribution over $d$-regular graphs on
$n$ labeled vertices. Sampling is non-trivial but well-approximated
by the \emph{configuration model}.
\end{definition}

\begin{definition}[Threshold function]
A function $t(n)$ is a \textbf{threshold} for a monotone graph
property $P$ if
\[
  \Pr[G(n, p) \models P] \to
  \begin{cases} 0 & \text{if } p/t(n) \to 0, \\
                1 & \text{if } p/t(n) \to \infty. \end{cases}
\]
\end{definition}

\begin{theorem}[Bollob\'as--Thomason, 1987]\label{thm:bt}
Every non-trivial monotone graph property has a threshold function
(unique up to constants).
\end{theorem}

\begin{proof}[Proof sketch]
Let $q(n, p)$ be the probability that $G(n, p)$ has $P$. Monotonicity
of $P$ and a coupling argument show $q$ is non-decreasing in $p$.
Define $t(n)$ by $q(n, t(n)) = 1/2$. A two-round argument with
$k = C \log(1/\delta)$ independent copies shows that boosting $p$ to
$kt(n)$ makes $q$ exceed $1 - \delta$ while halving $p$ below
$t(n)/k$ makes $q$ drop below $\delta$.
\end{proof}

\subsubsection{Subgraph Thresholds}

\begin{definition}[Density and balanced graph]
The \textbf{density} of $H$ is $m(H) = |E(H)|/|V(H)|$. The maximum
density of a subgraph of $H$ is $m^*(H) = \max_{H' \subseteq H} m(H')$.
A graph $H$ is \textbf{balanced} if $m(H) = m^*(H)$ and \textbf{strictly balanced}
if $m(H') < m(H)$ for all proper subgraphs $H'$.
\end{definition}

\begin{theorem}[Bollob\'as, 1981]\label{thm:bollobas-subgraph}
For every fixed graph $H$ with at least one edge, the property ``$H$
is a subgraph of $G$'' has threshold $t(n) = n^{-1/m^*(H)}$.
\end{theorem}

\begin{proof}[Proof sketch]
Let $H'$ be the densest subgraph of $H$. The expected number of copies
of $H'$ in $G(n, p)$ is $\Theta(n^{|V(H')|} p^{|E(H')|})$. This
tends to $0$ when $p \ll n^{-1/m(H')}$ and to $\infty$ otherwise.
First-moment gives the $0$-statement; second-moment (a moderately
involved variance computation) gives the $1$-statement.
\end{proof}

\begin{example}
\mbox{}
\begin{itemize}
  \item The threshold for containing $K_3$ is $p = 1/n$.
  \item The threshold for containing $K_4$ is $p = n^{-2/3}$.
  \item The threshold for containing any tree on $k$ vertices is $p = n^{-k/(k - 1)}$.
  \item The threshold for containing a Hamilton cycle is
        $p = \log n/n$---the same as connectivity.
\end{itemize}
\end{example}

\subsubsection{Degree Distribution}

\begin{theorem}[Poissonization of low degrees]
For $p = c/n$ with $c > 0$ fixed, the degree sequence of $G(n, p)$
converges in distribution to i.i.d.\ $\mathrm{Poisson}(c)$: for any
fixed $k$, the number of vertices of degree $k$ is
$(1 + o(1)) n e^{-c} c^k/k!$ with high probability.
\end{theorem}

\begin{corollary}[Maximum and minimum degree]
For $p = c/n$ with $c > 0$ fixed, $\Delta(G(n, p)) = (1 + o(1))\log n / \log\log n$ whp.
\end{corollary}

\subsubsection{Threshold for Connectivity}

\begin{theorem}[Erd\H{o}s--R\'enyi, 1960]\label{thm:connthresh}
Let $p = (\log n + c_n)/n$. Then
\[
  \Pr[G(n, p) \text{ is connected}] \;\to\;
  \begin{cases} 0 & \text{if } c_n \to -\infty, \\
                e^{-e^{-c}} & \text{if } c_n \to c \in \mathbb R, \\
                1 & \text{if } c_n \to \infty. \end{cases}
\]
\end{theorem}

\begin{proof}[Proof sketch]
\emph{Isolated vertices.} Let $X_0$ count isolated vertices.
$\mathbb{E}[X_0] = n(1 - p)^{n - 1} = e^{\log n - pn + O(1/n)} = e^{-c_n + o(1)}$.
Second-moment analysis gives $\mathrm{Var}(X_0) = (1 + o(1))\mathbb{E}[X_0]$,
so $X_0$ is asymptotically Poisson$(e^{-c})$; in particular
$\Pr[X_0 = 0] \to e^{-e^{-c}}$.

\emph{Connected iff no isolated.} Show that whp at $p \sim \log n/n$
the only obstruction to connectivity is isolated vertices: any
component of size $2 \le k \le n/2$ requires too many edges, and by
union bound over $k$ the probability tends to $0$.
\end{proof}

\begin{corollary}[Koml\'os--Szemer\'edi; Bollob\'as]
The thresholds for connectivity and Hamilton cycle coincide at
$p = \log n/n$.
\end{corollary}

\subsubsection{The Giant Component Phase Transition}

\begin{theorem}[Erd\H{o}s--R\'enyi giant component]\label{thm:giant}
Set $p = c/n$. Let $C_{\max}(G(n, p))$ denote the largest component.
As $n \to \infty$, with high probability:
\begin{itemize}
  \item \textbf{Subcritical} ($c < 1$): $|C_{\max}| = \Theta(\log n)$,
        and all components have size $O(\log n)$.
  \item \textbf{Critical} ($c = 1$): $|C_{\max}| = \Theta(n^{2/3})$.
  \item \textbf{Supercritical} ($c > 1$): There is a unique giant
        component of size $(1 + o(1))\rho_c \cdot n$, where $\rho_c$
        is the unique positive root of $1 - \rho = e^{-c\rho}$, and
        all other components have size $O(\log n)$.
\end{itemize}
\end{theorem}

\begin{proof}[Proof sketch via branching processes]
Explore the component of a fixed vertex $v$ by breadth-first search.
At each step, when exploring a vertex, its number of new neighbors is
approximately $\mathrm{Binomial}(n - k, p) \approx \mathrm{Poisson}(c)$
for $k = o(n)$. This BFS approximates a Galton--Watson branching
process with offspring $\mathrm{Poisson}(c)$. When $c < 1$ the
branching process dies a.s.\ in finite time (with geometric tail),
giving $O(\log n)$ component size. When $c > 1$, with probability
$\rho_c$ the process survives, yielding a macroscopic component.

The concentration/uniqueness of the giant is proved by second-moment
methods: showing two ``branches'' of size $\gtrsim n$ must eventually
intersect.
\end{proof}

\begin{remark}[Critical window]
Bollob\'as, and independently \L uczak, identified the critical
window $p = (1 + \lambda n^{-1/3})/n$, inside which
$|C_{\max}|/n^{2/3}$ converges to a universal random variable. This
was later refined by Aldous (1997) to a continuum-tree limit
description.
\end{remark}

\subsubsection{Chromatic Number of $G(n, p)$}

\begin{theorem}[Bollob\'as, 1988]
For $p = 1/2$ fixed, $\chi(G(n, 1/2)) = (1 + o(1)) \dfrac{n}{2 \log_2 n}$.
\end{theorem}

\begin{proof}[Proof sketch]
The lower bound uses $\chi \ge n/\alpha$ together with
$\alpha(G(n, 1/2)) \sim 2\log_2 n$ whp. The matching upper bound
requires a clever greedy algorithm (``Bollob\'as's martingale'') that
iteratively removes independent sets of size near the maximum and
applies Azuma's inequality for concentration.
\end{proof}

\begin{theorem}[Shamir--Spencer two-point concentration]
For any $p$ with $p n^{5/6} \to \infty$, there exist integer-valued
sequences $a_n$ such that
$\chi(G(n, p)) \in \{a_n, a_n + 1\}$ whp.
\end{theorem}

\subsubsection{Hamiltonicity}

\begin{theorem}[P\'osa, 1976; Koml\'os--Szemer\'edi, 1983]
The threshold for $G(n, p)$ to contain a Hamilton cycle is
$p = (\log n + \log \log n + c_n)/n$. Specifically, if $c_n \to \infty$,
$G(n, p)$ is Hamiltonian whp; if $c_n \to -\infty$, it is not.
\end{theorem}

\begin{remark}
The proof introduces \emph{P\'osa's rotation--extension} technique,
which iteratively rotates a non-extendable path to find new endpoints,
combined with a ``no small bad sets'' lemma via expansion.
\end{remark}

\subsubsection{Random Regular Graphs}

\begin{theorem}[Wormald, 1999]
For any fixed $d \ge 3$, the random $d$-regular graph $G_{n, d}$ is
Hamiltonian whp, has chromatic number in $\{3, 4\}$ whp (for $d = 3$;
larger for larger $d$), and has spectral gap bounded away from zero
whp.
\end{theorem}

\begin{theorem}[Friedman, 2008]
For every $\varepsilon > 0$ and any fixed $d \ge 3$, the second-largest
absolute eigenvalue of $G_{n, d}$ satisfies
\[
  |\lambda|(G_{n, d}) \le 2\sqrt{d - 1} + \varepsilon \quad \text{whp}.
\]
In particular, random regular graphs are ``asymptotically Ramanujan''.
\end{theorem}

\subsubsection{Preferential Attachment and Scale-Free Models}

\begin{definition}[Barab\'asi--Albert model]
Start with a small seed graph. At each time step, add a new vertex
$v_t$ with $m$ edges; each new edge attaches to an existing vertex
$u$ with probability proportional to $d(u)$.
\end{definition}

\begin{theorem}[Bollob\'as--Riordan--Spencer--Tusn\'ady, 2001]
In the BA model with $m \ge 1$, the fraction of vertices with degree
exactly $k$ converges in probability to $c_m k^{-3}$ for each fixed
$k$: a power-law (\emph{scale-free}) degree distribution.
\end{theorem}

\subsubsection{Small Subgraph Central Limit Theorem}

\begin{theorem}[Rucinski, 1988]
Let $H$ be a fixed graph and let $X_H = X_H(n, p)$ count copies of
$H$ in $G(n, p)$. If $pn^{|V(H)|/|E(H)|} \to \infty$ and
$n^2(1 - p) \to \infty$, then
\[
  \frac{X_H - \mathbb{E}[X_H]}{\sqrt{\mathrm{Var}(X_H)}} \;\xrightarrow{d}\; \mathcal{N}(0, 1).
\]
\end{theorem}
...
\begin{exercise}
Use the first-moment method to prove that the threshold for
$G(n, p)$ to contain a triangle is $p = 1/n$: show
$\mathbb{E}[\#K_3] = \binom{n}{3} p^3$ and apply Markov.
\end{exercise}

\begin{exercise}
Compute the threshold for the appearance of $C_4$ in $G(n, p)$.
Answer: $p = n^{-1}$ (using $m^*(C_4) = 1$).
\end{exercise}

\begin{exercise}
Prove that the random $3$-regular graph is connected whp.
\end{exercise}

\paragraph{Medium Questions.}

\begin{exercise}
Prove that $\alpha(G(n, 1/2)) = (2 + o(1)) \log_2 n$ whp. (First
moment for the upper bound; second moment / alteration for the
matching lower bound.)
\end{exercise}

\begin{exercise}
Prove Theorem~\ref{thm:bt} (Bollob\'as--Thomason) assuming monotonicity
and using the Janson inequalities.
\end{exercise}

\begin{exercise}
Prove that for $p = 1/n$ (critical), the expected number of
\emph{tree} components of size $k$ in $G(n, p)$ is asymptotically
$n \cdot k^{k - 2} e^{-k}/k!$. (Use Cayley's formula.)
\end{exercise}

\paragraph{Hard Questions.}

\begin{exercise}
Prove that for $p n^{2/3} \to \infty$, the number of triangles in
$G(n, p)$ is asymptotically normal. (Variance computation plus
Stein's method or the method of moments.)
\end{exercise}

\begin{exercise}
Show that for $p = c/n$ with $c > 1$, the diameter of the giant
component of $G(n, p)$ is $(1 + o(1))\log n / \log c$ whp.
\end{exercise}

\begin{exercise}
Let $p = c/n$ with $c > 1$. Show that the fraction of vertices in
components of size $\le k$ converges to an explicit value involving
the Borel distribution.
\end{exercise}

\begin{exercise}
Use Azuma's inequality on the vertex-exposure martingale to show
$\chi(G(n, p))$ is concentrated within $O(\sqrt{n \log n})$ of its
median, for any $p$.
\end{exercise}
...
\begin{exercise}
Sketch the qualitative shape of the survival probability $\rho_c$
(the unique positive root of $1 - \rho = e^{-c\rho}$) as a function
of $c$ on $[0, 3]$. Mark the critical point $c = 1$ and the
horizontal asymptote $\rho_\infty = 1$. Annotate the picture with
the three regimes (subcritical / critical / supercritical) of
Theorem~\ref{thm:giant}.
\end{exercise}

\begin{exercise}
Draw a sample $G(8, 1/2)$ and identify by inspection: the largest
component, all isolated vertices, the maximum and minimum degree.
Compare with the expectations $\mathbb{E}[X_0] = 8 \cdot (1/2)^7$
and $\mathbb{E}[\Delta] \approx \log_2 8$.
\end{exercise}

\subsection{Szemer\'edi's Regularity Lemma}

Szemer\'edi's regularity lemma, first proved as a technical tool
toward his 1975 theorem on arithmetic progressions in dense sets,
has become one of the most important results in extremal
combinatorics. Intuitively it says: \emph{every large dense graph is
``close'' to a bounded-complexity weighted object}. Combined with the
\emph{counting lemma} (embedding counts match random predictions to
within error $\varepsilon$) and the \emph{graph removal lemma}, it provides
a general-purpose machine for translating structural statements
about $H$-free-ness into approximate statements about the ambient
graph.

\subsubsection{$\varepsilon$-Regular Pairs}

\begin{definition}[Density]
For disjoint non-empty $X, Y \subseteq V(G)$, the \textbf{edge density}
is $d(X, Y) = e(X, Y)/(|X||Y|)$, where $e(X, Y)$ is the number of
edges with one endpoint in each.
\end{definition}

\begin{definition}[$\varepsilon$-regular pair]\label{def:regular-pair}
A pair $(X, Y)$ of disjoint non-empty vertex sets is
$\varepsilon$-\textbf{regular} if for every $X' \subseteq X$ with
$|X'| \ge \varepsilon|X|$ and every $Y' \subseteq Y$ with
$|Y'| \ge \varepsilon|Y|$,
\[
  |d(X', Y') - d(X, Y)| \;\le\; \varepsilon.
\]
\end{definition}

\begin{remark}[Pseudorandomness]
A regular pair $(X, Y)$ is a ``pseudorandom'' bipartite graph: its
edges are spread so uniformly that no large sub-rectangle has density
deviating significantly from the average. In particular, random
bipartite graphs are regular (with high probability), but so are many
deterministic constructions such as Paley graphs.
\end{remark}

\begin{proposition}[Sliceability]
If $(X, Y)$ is $\varepsilon$-regular with density $d$, $X' \subseteq X$ with
$|X'| \ge \varepsilon |X|$, $Y' \subseteq Y$ with $|Y'| \ge \varepsilon |Y|$,
then $(X', Y')$ is $2\varepsilon$-regular with density in $[d - \varepsilon, d + \varepsilon]$.
\end{proposition}

\begin{lemma}[Typical degree in a regular pair]\label{lem:regulartypical}
If $(X, Y)$ is $\varepsilon$-regular with density $d$, then all but at
most $\varepsilon|X|$ vertices $x \in X$ have $|N(x) \cap Y| \in [(d - \varepsilon)|Y|, (d + \varepsilon)|Y|]$.
\end{lemma}

\begin{proof}
Suppose more than $\varepsilon|X|$ vertices have $|N(x) \cap Y| < (d - \varepsilon)|Y|$.
Let $X'$ be that exceptional set. Then $d(X', Y) < d - \varepsilon$, but
$|X'| \ge \varepsilon|X|, |Y| \ge \varepsilon|Y|$, contradicting $\varepsilon$-regularity.
\end{proof}

\subsubsection{The Regularity Lemma}

\begin{definition}[Equipartition]
An \textbf{equipartition} of $V(G)$ into $k$ parts is a partition
$V = V_1 \cup V_2 \cup \cdots \cup V_k$ where $||V_i| - |V_j|| \le 1$
for all $i, j$.
\end{definition}

\begin{definition}[$\varepsilon$-regular partition]
A partition $V = V_0 \cup V_1 \cup \cdots \cup V_k$ is
$\varepsilon$-\textbf{regular} if $|V_0| \le \varepsilon n$, the parts
$V_1, \dots, V_k$ are equipartitioned, and at most $\varepsilon k^2$ pairs
$(V_i, V_j)$ fail to be $\varepsilon$-regular.
\end{definition}

\begin{theorem}[Szemer\'edi Regularity Lemma, 1975]\label{thm:srl}
For every $\varepsilon > 0$ and $m_0 \ge 1$ there exists $M = M(\varepsilon, m_0)$
such that every graph $G$ with at least $m_0$ vertices admits an
...
\begin{tikzpicture}[
    scale=0.85,
    cell/.style={draw, minimum width=1.5cm, minimum height=1.0cm, inner sep=0pt},
    iregular/.style={draw, fill=red!18, minimum size=0.45cm, inner sep=0pt},
    regular/.style={draw, fill=green!18, minimum size=0.45cm, inner sep=0pt}
]
    % 5x5 grid of pairs (only upper triangle drawn) representing density matrix
    \node[cell, fill=gray!10] (V0) at (-3.5, 2.0) {\small $V_0$};
    \foreach \i in {1,2,3,4} {
        \node[cell] (V\i) at ({-3.5 + 1.5*\i}, 2.0) {\small $V_{\i}$};
    }

    % Density matrix: rows V_1..V_4, cols V_1..V_4
    \matrix[matrix of nodes, nodes={iregular}, column sep=2pt, row sep=2pt] (M) at (0, -0.3) {
       |[regular]| 0.35 & |[regular]| 0.62 & |[iregular]| ?? & |[regular]| 0.41 \\
       |[regular]| 0.62 & |[regular]| 0.18 & |[regular]| 0.55 & |[regular]| 0.30 \\
       |[iregular]| ??  & |[regular]| 0.55 & |[regular]| 0.27 & |[regular]| 0.48 \\
       |[regular]| 0.41 & |[regular]| 0.30 & |[regular]| 0.48 & |[regular]| 0.39 \\
    };

    \node at (0, -2.4) {\small Density matrix $d(V_i, V_j)$ for the partition.};
    \node at (0, -2.9) {\small Green: $\varepsilon$-regular pair. \ Red ``??'': irregular pair (counted toward $\le \varepsilon k^2$ budget).};
    \node at (0, -3.5) {\small $V_0$ is the small ``garbage'' set, $|V_0| \le \varepsilon n$.};
\end{tikzpicture}
\\
\small Figure 204: An $\varepsilon$-regular partition of $V(G)$ into $V_0 \cup V_1 \cup \cdots \cup V_4$.
\label{fig:regularity-partition}
\end{figure}

\begin{proof}[Proof via mean-square density / energy increment]
Define the \emph{energy} (or \emph{index}) of a partition $\mathcal P = \{V_1, \dots, V_k\}$:
\[
  q(\mathcal P) \;=\; \sum_{i, j} \frac{|V_i| |V_j|}{n^2} \cdot d(V_i, V_j)^2.
\]
Note $0 \le q(\mathcal P) \le 1$ since densities lie in $[0, 1]$.
Key lemma (\emph{energy increment}): if a partition has more than
$\varepsilon k^2$ irregular pairs, one can refine it to a partition
$\mathcal P'$ (with each $V_i$ split into $\le 2^k$ smaller sets) such
that $q(\mathcal P') \ge q(\mathcal P) + \varepsilon^5$.

Starting from any initial partition, iterate. Since energy increases
by $\varepsilon^5$ each step and is bounded by $1$, the process terminates
in $\le \varepsilon^{-5}$ steps, yielding the claimed bound on $k$. The
partition size bound is a tower of $2$'s of height $O(\varepsilon^{-5})$.
\end{proof}

\begin{remark}[Gowers's lower bound]
The tower-type dependence $M = M(\varepsilon)$ is essentially unavoidable:
Gowers (1997) constructed graphs for which every $\varepsilon$-regular
partition has $k \ge \mathrm{tower}(1/\varepsilon^{1/16})$.
\end{remark}

\subsubsection{The Counting Lemma}

\begin{theorem}[Counting Lemma]\label{thm:counting}
Let $H$ be a graph with vertex set $\{1, \dots, h\}$ and $m$ edges.
Suppose $V_1, \dots, V_h$ are vertex sets in $G$ such that for every
$ij \in E(H)$, the pair $(V_i, V_j)$ is $\varepsilon$-regular with density
$d_{ij} \ge 2\varepsilon$. Then the number of labeled copies of $H$ in $G$
with vertex $i$ in $V_i$ is
\[
  N(H; V_1, \dots, V_h) \;=\; \Big(1 \pm O(\varepsilon)\Big)
     \Bigg( \prod_{ij \in E(H)} d_{ij} \Bigg) \prod_{i = 1}^{h} |V_i|.
\]
\end{theorem}

\begin{proof}[Proof sketch by induction on $h$]
Embed vertices of $H$ into $V_1, \dots, V_h$ greedily. For each choice
of $V_1, \dots, V_{i - 1}$ embedded, Lemma~\ref{lem:regulartypical}
controls the typical common neighborhood intersection in $V_i$ to
within factor $(1 \pm O(\varepsilon))$; multiply across all $m$ edges.
\end{proof}

\subsubsection{The Removal Lemma}

\begin{theorem}[Triangle Removal Lemma; Ruzsa--Szemer\'edi, 1976]\label{thm:triangleremoval}
For every $\eta > 0$ there exists $\delta > 0$ such that if a graph
$G$ on $n$ vertices contains at most $\delta n^3$ triangles, then
$G$ can be made triangle-free by removing at most $\eta n^2$ edges.
\end{theorem}

\begin{proof}
Apply Theorem~\ref{thm:srl} with $\varepsilon = \eta/4$ to obtain a regular
partition $V_1, \dots, V_k$. Remove: (a) all edges inside any single
part, (b) all edges between pairs that are not $\varepsilon$-regular, (c)
all edges across regular pairs of density $< 2\varepsilon$. Total edges
removed: at most $\eta n^2$. Now claim the resulting graph is
triangle-free. Otherwise a triangle $v_1 v_2 v_3$ with
$v_i \in V_{i'}$ survives; then $V_{1'}, V_{2'}, V_{3'}$ form a
triangle of regular-dense pairs, and by
Theorem~\ref{thm:counting} there are $\ge c \cdot n^3$ triangles in
$G$ for some $c = c(\eta)$. Choose $\delta < c$.
\end{proof}

\begin{theorem}[General Graph Removal Lemma; Erd\H{o}s--Frankl--R\"odl]
For every graph $H$ and every $\eta > 0$ there exists $\delta > 0$
such that if $G$ on $n$ vertices has fewer than $\delta n^{|V(H)|}$
copies of $H$, then at most $\eta n^2$ edges need be removed to make
$G$ $H$-free.
\end{theorem}

\subsubsection{From Triangle Removal to Roth}

\begin{theorem}[Roth, 1953]\label{thm:roth}
For every $\alpha > 0$ there exists $N_0$ such that every subset of
$\{1, 2, \dots, N\}$ with $N \ge N_0$ and at least $\alpha N$
elements contains a non-trivial $3$-term arithmetic progression.
\end{theorem}

\begin{proof}[Proof via triangle removal; Ruzsa]
Let $A \subseteq [N]$ have density $\alpha$. Build the \emph{Ruzsa
3-AP graph} $G_A$ with vertex set $X \cup Y \cup Z$ where each copy
is $[3N]$. Edges: $(x, y) \in X \times Y$ iff $y - x \in A$;
$(y, z) \in Y \times Z$ iff $z - y \in A$; $(x, z) \in X \times Z$
iff $(z - x)/2 \in A$. Triangles $(x, y, z)$ with $x + z = 2y$
correspond to $3$-APs $(y - x, y, y + (z - y))$ within $A$.

$G_A$ has $9 N^2$ vertices and $\Theta(\alpha N^2)$ edges, and
trivial triangles (all three vertices encoding the same AP element)
give $\Theta(\alpha N^2)$ vertex-disjoint triangles. By
Theorem~\ref{thm:triangleremoval}, any edge-removal making $G_A$
triangle-free requires $\Omega(N^2)$ edges, so $G_A$ must have
$\Omega(N^3)$ triangles---yielding a non-trivial 3-AP in $A$.
\end{proof}

\subsubsection{The Blow-up Lemma}

\begin{theorem}[Koml\'os--S\'ark\"ozy--Szemer\'edi blow-up lemma, 1997]
Let $H$ be a graph of bounded maximum degree $\Delta$ and $d \in (0, 1)$.
If $G$ contains a ``super-regular''  blown-up version of an auxiliary
graph $R$, then every graph $H$ with $\chi(H) \le r$ and $\Delta(H) \le \Delta$
can be embedded into $G$, provided $|V(H)| \le |V(G)|$ and
$r \le |V(R)|$.
\end{theorem}

The blow-up lemma is the counterpart of the counting lemma when one
wants to embed a \emph{spanning} or nearly-spanning subgraph rather
than count all copies of a fixed $H$. It is the key ingredient in
P\'osa-style extremal-chromatic theorems and in the resolution of
the Bollob\'as--Koml\'os tiling conjecture.

\subsubsection{Graphons and the Limit Theory}

\begin{definition}[Graphon]
A \textbf{graphon} is a symmetric measurable function
$W : [0, 1]^2 \to [0, 1]$. Every graph $G$ defines a graphon $W_G$
(the indicator of its adjacency matrix rescaled), and the space of
graphons carries a natural \emph{cut-distance} metric.
\end{definition}

\begin{theorem}[Lov\'asz--Szegedy; Borgs--Chayes et al., 2006]
The space of graphons modulo measure-preserving relabelings is
compact under cut-distance, and any sequence of graphs with
convergent subgraph densities converges to some graphon limit.
\end{theorem}

Graphons are the ``real-valued'' limit objects encoding what large
graphs look like; they underlie the modern theory of graph property
testing and extremal graph theory.

\subsubsection{Property Testing}

\begin{theorem}[Alon--Shapira: testability of hereditary properties]
Every hereditary graph property $\mathcal P$ is \emph{testable} with
one-sided error: there is an algorithm which, given oracle access to
a graph $G$ and a parameter $\varepsilon$, queries $O(\mathrm{poly}(1/\varepsilon))$
edges of $G$ and (a) accepts if $G \in \mathcal P$, (b) rejects with
probability $\ge 2/3$ if $G$ is $\varepsilon$-far from $\mathcal P$.
\end{theorem}

The proof relies directly on the regularity lemma and the
graph-removal lemma: a large graph $\varepsilon$-far from $\mathcal P$ must
contain ``many'' forbidden subgraphs, detectable by sampling.
...
\begin{exercise}
Prove the \emph{symmetry} of regularity: if $(X, Y)$ is $\varepsilon$-regular
with density $d$, then so is $(Y, X)$.
\end{exercise}

\begin{exercise}
Show that a random bipartite graph with edge probability $p$ is
$\varepsilon$-regular with high probability for any fixed $\varepsilon > 0$.
\end{exercise}

\begin{exercise}
Using the counting lemma, give a short proof of the Erd\H{o}s--Stone
theorem.
\end{exercise}
...
\begin{exercise}
Prove that the triangle removal lemma implies the corner theorem
(Ajtai--Szemer\'edi): every subset of $[N]^2$ of positive density
contains a \emph{corner} $\{(x, y), (x + d, y), (x, y + d)\}$.
\end{exercise}

\begin{exercise}
Prove Mantel's theorem via the triangle removal lemma: every graph
with more than $(1/4 + \varepsilon) n^2$ edges contains $\Omega(n^3)$
triangles, contradicting $\mathrm{ex}(n, K_3) \le n^2/4$.
\end{exercise}

\begin{exercise}
Show that the graph removal lemma is equivalent to the following
statement: for any fixed $H$ and $\varepsilon > 0$, every graph with
fewer than $\delta n^{|V(H)|}$ copies of $H$ contains an
$H$-free spanning subgraph with at least $e(G) - \varepsilon n^2$ edges.
\end{exercise}

\begin{exercise}
Using the regularity lemma, give a proof of the Erd\H{o}s--Simonovits
stability theorem.
\end{exercise}
...
\begin{exercise}
Using the triangle removal lemma, prove the following Behrend-type
complement: there exist subsets of $[N]$ of size $N/\exp(\sqrt{\log N})$
with no 3-AP. Conclude that $\delta(\eta)$ in the removal lemma must
satisfy $\delta \le \exp(-\mathrm{poly}(1/\eta))$.
\end{exercise}

\begin{exercise}
Prove the hypergraph version: for $3$-uniform hypergraphs, every
$H$ with few copies can be made $H$-free by deleting few edges.
(Use the hypergraph regularity lemma of Gowers / R\"odl--Schacht.)
\end{exercise}

\begin{exercise}
Show that cut-distance $d_\square(W_1, W_2)$ is a metric on the space
of graphons modulo measure-preserving relabelings, and that it is
compatible with subgraph densities in the sense of Lov\'asz--Szegedy.
\end{exercise}

\paragraph{Diagram Questions.}

\begin{exercise}
For the graph $G$ obtained as the disjoint union of $K_{4,4}$ and an
independent set of $4$ vertices, construct an $\varepsilon$-regular
partition with $\varepsilon = 0.1$ into $k = 3$ parts (the two sides of
the biclique and the independent set). Tabulate the $3 \times 3$
density matrix as in Figure~\ref{fig:regularity-partition} and verify
that all pairs are $0.1$-regular.
\end{exercise}

\begin{exercise}
Draw the Ruzsa $3$-AP graph $G_A$ for $A = \{1, 4\} \subset [5]$.
Identify all triangles of $G_A$ and verify that the trivial triangles
correspond to the elements $1$ and $4$ of $A$ themselves, and that
$A$ contains no $3$-AP.
\end{exercise}

\subsection{Graph Minors and Treewidth}

The graph minor programme of Robertson and Seymour (1983--2011) is
arguably the deepest body of work in structural graph theory. The
two central pillars are the \textbf{Graph Minor Theorem}
(well-quasi-ordering of finite graphs under the minor relation) and
the \textbf{Graph Minor Structure Theorem} (every graph excluding a
fixed minor is nearly planar in a precise sense). Alongside these,
\textbf{treewidth} has emerged as the single most important
parameter for measuring a graph's ``distance to being a tree'' and
the crux of parameterized algorithm design.

\subsubsection{Minors and Topological Minors}

\begin{definition}[Minor]\label{def:minor}
A graph $H$ is a \textbf{minor} of $G$, written $H \preceq G$, if $H$
can be obtained from a subgraph of $G$ by contracting edges.
Equivalently, there exist disjoint connected vertex sets
$\{V_u : u \in V(H)\} \subseteq V(G)$ (the \emph{branch sets}) such
that whenever $uv \in E(H)$, some edge of $G$ goes between $V_u$ and
$V_v$. Each branch set $V_u$ induces a connected subgraph of $G$.
\end{definition}

\begin{definition}[Topological minor]
$H$ is a \textbf{topological minor} of $G$ if some subdivision of $H$
is a subgraph of $G$. Every topological minor is a minor; the converse
fails (e.g.\ $K_5$ is a minor but not a topological minor of the
Petersen graph).
\end{definition}

\begin{proposition}[Subdivisions and maximum degree]
$H$ is a topological minor of $G$ iff $H$ is a minor of $G$ via branch
sets that are paths. If $\Delta(H) \le 3$, topological-minor and minor
coincide.
\end{proposition}

\begin{theorem}[Kuratowski, 1930]\label{thm:kuratowski}
A graph $G$ is planar iff it contains no subdivision of $K_5$ or
$K_{3, 3}$.
\end{theorem}

\begin{theorem}[Wagner, 1937]\label{thm:wagner}
A graph $G$ is planar iff neither $K_5$ nor $K_{3, 3}$ is a minor of
$G$.
\end{theorem}

\subsubsection{Tree Decompositions and Treewidth}

\begin{definition}[Tree decomposition]\label{def:tw}
A \textbf{tree decomposition} of $G$ is a pair $(T, \mathcal B)$ where
$T$ is a tree and $\mathcal B = (B_t)_{t \in V(T)}$ is a family of
\textbf{bags} $B_t \subseteq V(G)$ such that:
\begin{enumerate}
  \item $\bigcup_{t \in V(T)} B_t = V(G)$;
  \item for each edge $uv \in E(G)$ there is $t$ with $\{u, v\} \subseteq B_t$;
  \item for each $v \in V(G)$, the set $\{t \in V(T) : v \in B_t\}$
        induces a non-empty subtree of $T$ (the \textbf{interpolation
        property}).
\end{enumerate}
The \textbf{width} of $(T, \mathcal B)$ is $\max_{t} |B_t| - 1$; the
\textbf{treewidth} of $G$ is
$\mathrm{tw}(G) = \min\{\text{width of a tree decomposition of } G\}$.
\end{definition}

\begin{example}[Small treewidth]
\mbox{}
\begin{itemize}
  \item $\mathrm{tw}(T) = 1$ for any tree $T$ with $\ge 2$ vertices.
  \item $\mathrm{tw}(C_n) = 2$ for $n \ge 3$.
  \item $\mathrm{tw}(K_n) = n - 1$.
  \item $\mathrm{tw}$(series--parallel graph) $\le 2$.
  \item $\mathrm{tw}$(planar graph on $n$ vertices) $= O(\sqrt n)$.
  \item $\mathrm{tw}(n \times n \text{ grid}) = n$.
\end{itemize}
\end{example}
...
\begin{tikzpicture}[
    scale=1.0,
    vertex/.style={circle, draw, fill=white, minimum size=0.5cm, inner sep=1pt},
    bag/.style={rectangle, draw, rounded corners=4pt, fill=blue!8, inner sep=4pt, minimum height=0.7cm},
    edge/.style={line width=0.7pt}
]
    % LEFT: a small graph G (cycle C_4 with chord)
    \begin{scope}[xshift=-4cm]
        \node[vertex] (a) at (0,1) {\small $a$};
        \node[vertex] (b) at (1.6,1) {\small $b$};
        \node[vertex] (c) at (1.6,-0.6) {\small $c$};
        \node[vertex] (d) at (0,-0.6) {\small $d$};
        \draw[edge] (a) -- (b);
        \draw[edge] (b) -- (c);
        \draw[edge] (c) -- (d);
        \draw[edge] (d) -- (a);
        \draw[edge] (a) -- (c); % chord
        \node at (0.8,-1.6) {\small Graph $G$};
    \end{scope}

    % RIGHT: tree decomposition (path of 2 bags)
    \begin{scope}[xshift=2cm]
        \node[bag] (B1) at (0,0.5) {$\{a, b, c\}$};
        \node[bag] (B2) at (3,0.5) {$\{a, c, d\}$};
        \draw[edge] (B1) -- (B2);
        \node at (1.5,-1.6) {\small Tree decomposition $(T, \mathcal B)$ of width $2$};
        \node at (1.5,-2.2) {\small $\mathrm{tw}(G) = 2$};
    \end{scope}
\end{tikzpicture}
\\
\small Figure 205: A graph $G$ with chord $ac$ and a width-$2$ tree decomposition. The vertex $a$ (and likewise $c$) appears in both bags; this is allowed by the interpolation property because the bags containing $a$ form a (sub)tree.
\label{fig:treewidth}
\end{figure}

\begin{proposition}[Subadditivity under separators]\label{prop:separator}
If $S$ is a separator of $G$ (i.e.\ $G \setminus S$ is disconnected,
say with components inducing subgraphs $G_1, \dots, G_k$), then
\[
  \mathrm{tw}(G) \;\le\; \max_{i}\, \mathrm{tw}(G_i \cup S) + |S|.
\]
More precisely, tree decompositions glue along $S$.
\end{proposition}

\begin{theorem}[Pathwidth and intervals]
A graph $G$ has pathwidth at most $k$ iff $G$ is a subgraph of an
\emph{interval graph} with clique number $\le k + 1$.
\end{theorem}

\begin{theorem}[Treewidth and chordal graphs]\label{thm:chordaltw}
$\mathrm{tw}(G) \le k$ iff $G$ is a subgraph of a chordal graph with
clique number $\le k + 1$.
\end{theorem}

\begin{proof}[Proof sketch]
$(\Leftarrow)$ Chordal graphs have tree decompositions whose bags are
maximal cliques.

$(\Rightarrow)$ Given a tree decomposition, add edges between any two
vertices sharing a bag; the resulting graph is chordal (clique trees
exist).
\end{proof}

\subsubsection{Duality: Brambles and Cops--Robbers}

\begin{definition}[Bramble]\label{def:bramble}
A \textbf{bramble} in $G$ is a family $\mathcal{B}$ of connected
subgraphs such that any two elements either share a vertex or are
connected by an edge. A \textbf{hitting set} for $\mathcal{B}$ is a
vertex set meeting every element. The \textbf{bramble number}
$\mathrm{bn}(G)$ is the maximum size of the minimum hitting set over
all brambles.
\end{definition}

\begin{theorem}[Seymour--Thomas, 1993]\label{thm:brambleduality}
$\mathrm{tw}(G) + 1 \;=\; \mathrm{bn}(G)$.
\end{theorem}

This min--max duality is the graph-theoretic analogue of LP duality
and is the most convenient tool for proving \emph{lower bounds} on
treewidth.

\begin{definition}[Cops and robber game]
In the $k$-cop game on $G$, $k$ cops and a robber are placed on
vertices. Each round: cops may jointly teleport to any vertices, and
the robber runs along any $(V(G) \setminus \text{new cops})$-avoiding
path. Cops win if a cop ever lands on the robber. The
\textbf{cop number} is the least $k$ for which cops win.
\end{definition}

\begin{theorem}[Seymour--Thomas]\label{thm:copstw}
In the \emph{visible-robber} variant of the cops game,
$k$ cops win on $G$ iff $\mathrm{tw}(G) \le k - 1$.
\end{theorem}

\subsubsection{Courcelle's Theorem and Fixed-Parameter Tractability}

\begin{definition}[MSO$_2$ logic]
$\mathrm{MSO}_2$ graph logic allows variables for vertices, edges,
sets of vertices, and sets of edges, together with the membership
relations and a binary predicate $\mathrm{inc}(v, e)$ for ``$v$ is
incident to $e$''. Most natural graph properties---$k$-colorability,
Hamiltonicity, existence of a given minor, etc.---are expressible in
$\mathrm{MSO}_2$.
\end{definition}

\begin{theorem}[Courcelle, 1990]\label{thm:courcelle}
Every graph property expressible in $\mathrm{MSO}_2$ can be decided in
time $f(k, \phi) \cdot n$ on graphs of treewidth at most $k$, where
$\phi$ is the $\mathrm{MSO}_2$ formula and $f$ is a computable (but
typically tower-type) function.
\end{theorem}

\begin{corollary}
$3$-coloring, Hamilton cycle, vertex cover, independent set, and
minor containment are all solvable in linear time on graphs of
bounded treewidth.
\end{corollary}

\begin{remark}[Practical tractability]
Bodlaender's algorithm (1996) computes a tree decomposition of width
$\le k$ in time $2^{O(k^3)} n$; many MSO$_2$ problems then admit
explicit DP algorithms with single-exponential dependence on $k$.
\end{remark}

\subsubsection{Excluded Minors and Sparsity}

\begin{theorem}[Mader; Thomason; Kostochka]
Every graph with average degree at least $c k \sqrt{\log k}$ contains
$K_k$ as a minor, where $c$ is an absolute constant. Moreover the
bound is tight up to the constant.
\end{theorem}

\begin{theorem}[Hadwiger's conjecture]
Every graph $G$ of chromatic number $k$ contains $K_k$ as a minor.
\end{theorem}

Hadwiger is an enormous strengthening of the Four Color Theorem
(proved for $k = 4$ by Wagner, $k = 5$ by Robertson--Seymour--Thomas,
and open for $k \ge 6$ in general). Recent progress by Norin--Postle--Song
gives $O(k (\log \log k)^6)$-chromatic suffices for $K_k$-minor.

\subsubsection{The Graph Minor Structure Theorem}

\begin{theorem}[Robertson--Seymour Grid Minor Theorem, 1986]\label{thm:gridminor}
For every $k$ there exists $f(k)$ such that every graph of treewidth
at least $f(k)$ contains the $k \times k$ grid as a minor.
Quantitatively, Chuzhoy and Tan showed $f(k) = O(k^9 \mathrm{polylog}(k))$.
\end{theorem}

\begin{theorem}[Graph Minor Structure Theorem; Robertson--Seymour, 2003]
For every graph $H$, every graph $G$ with no $H$-minor admits a
tree decomposition into ``almost-embeddable'' pieces: each bag is
obtained from a graph drawable on a surface of bounded genus by
adding a bounded number of \emph{vortices} (local perturbations) and
\emph{apex} vertices.
\end{theorem}

The Structure Theorem is the core technical tool underlying both the
proof of the Graph Minor Theorem and the design of FPT algorithms for
minor-closed problems (e.g.\ disjoint paths).

\subsubsection{The Graph Minor Theorem}

\begin{definition}[Well-quasi-order]
A relation $\preceq$ on a set $X$ is a \textbf{well-quasi-order} if
it is reflexive, transitive, and every infinite sequence
$x_1, x_2, x_3, \dots$ contains indices $i < j$ with $x_i \preceq x_j$.
Equivalently, there is no infinite antichain and no infinite strictly
decreasing sequence.
\end{definition}

\begin{theorem}[Higman's lemma]
If $(X, \preceq)$ is a wqo, then so is $(X^*, \preceq^*)$ on finite
sequences under the subsequence embedding.
\end{theorem}

\begin{theorem}[Kruskal's tree theorem, 1960]
Finite rooted trees form a wqo under the \emph{homeomorphic
embedding} relation.
\end{theorem}

\begin{theorem}[Robertson--Seymour Graph Minor Theorem, 1983--2004]\label{thm:robertsonseymour}
Finite graphs form a well-quasi-order under the minor relation:
in any infinite sequence $G_1, G_2, \dots$ there exist $i < j$ with
$G_i \preceq G_j$.
\end{theorem}

\begin{corollary}[Finite obstruction sets]\label{cor:finiteobstruction}
Every minor-closed graph property $\mathcal{P}$ is characterized by a
\emph{finite} list of forbidden minors: there exist
$H_1, \dots, H_k \notin \mathcal P$ such that $G \in \mathcal P$ iff
no $H_i$ is a minor of $G$.
\end{corollary}

\begin{proof}
Let $\mathrm{Obs}(\mathcal P)$ be the set of minor-minimal graphs not
in $\mathcal P$. Any two distinct elements are incomparable (else the
smaller one would contradict minimality), so $\mathrm{Obs}$ is an
antichain. By Theorem~\ref{thm:robertsonseymour} antichains are
finite.
\end{proof}

\begin{corollary}[Algorithmic]
For every fixed minor $H$, there is an algorithm deciding in time
$O(n^3)$ whether $H$ is a minor of a given graph $G$. Combined with
Corollary~\ref{cor:finiteobstruction}, every minor-closed property
admits a cubic-time recognition algorithm---even when the obstruction
set is unknown!
\end{corollary}

\begin{remark}
The Graph Minor Theorem spans more than 500 pages across twenty
papers. The proof relies on the Structure Theorem plus an intricate
induction over partial orderings of embedded structures. The cubic
running time for minor testing is attributed to Robertson--Seymour
for fixed $H$ and has since been refined.
\end{remark}

\subsubsection{The Disjoint Paths Problem}

\begin{theorem}[Robertson--Seymour disjoint paths]
For every fixed $k$, the \textbf{$k$-disjoint paths problem}---given
$G$ and pairs $(s_1, t_1), \dots, (s_k, t_k)$, determine whether there
exist vertex-disjoint paths $P_i$ from $s_i$ to $t_i$---is solvable
in time $O(n^3)$.
\end{theorem}

This is a major consequence of the Structure Theorem: while the
problem is $\mathrm{NP}$-hard when $k$ is part of the input, any
fixed number of terminal pairs can be handled in polynomial time.
...
\begin{exercise}
Show that every graph with treewidth at most $k$ has at most $kn$
edges. Deduce that planar graphs, despite allowing up to $3n - 6$
edges, may still have large treewidth.
\end{exercise}

\begin{exercise}
Compute the treewidth of the Petersen graph.
\end{exercise}

\begin{exercise}
Verify directly that the cycle $C_5$ has treewidth $2$ by exhibiting
...
\begin{exercise}
Prove that $\mathrm{tw}(G) \le k$ iff $G$ has a chordal supergraph
with clique number $\le k + 1$ (Theorem~\ref{thm:chordaltw}).
\end{exercise}

\begin{exercise}
Show that $K_{3, 3}$ and $K_5$ are the only two minor-minimal
non-planar graphs, completing Wagner's theorem
(Theorem~\ref{thm:wagner}).
\end{exercise}

\begin{exercise}
Prove Kruskal's tree theorem from Higman's lemma.
\end{exercise}

\begin{exercise}
Show that the class of \emph{series--parallel} graphs (graphs with no
$K_4$-minor) coincides with graphs of treewidth at most $2$.
\end{exercise}

\begin{exercise}
Using Courcelle's theorem, show that $3$-coloring is solvable in
linear time on graphs of bounded treewidth. Construct an explicit DP
algorithm with running time $O(3^k \cdot k \cdot n)$.
\end{exercise}

\paragraph{Hard Questions.}

\begin{exercise}
Prove that the $n \times n$ grid graph has treewidth exactly $n$.
(The upper bound is easy via a sweeping decomposition; the lower
bound uses Theorem~\ref{thm:brambleduality}.)
\end{exercise}

\begin{exercise}
Prove Hadwiger's conjecture for $k = 4$: every graph of chromatic
number $4$ contains $K_4$ as a minor. (Use an inductive
decomposition.)
\end{exercise}

\begin{exercise}
Prove that every graph with no $K_t$-minor has chromatic number
$O(t \sqrt{\log t})$. (Use Mader's density bound.)
\end{exercise}

\begin{exercise}
Show that well-quasi-ordering of finite graphs under \emph{topological
minor} fails: construct an infinite antichain of graphs under that
relation.
\end{exercise}

\begin{exercise}
Prove the grid-minor theorem (Theorem~\ref{thm:gridminor}) with the
weaker bound $f(k) = 2^{\Theta(k)}$, using an induction on treewidth
and the separator / tangle machinery.
\end{exercise}

\paragraph{Diagram Questions.}

\begin{exercise}
For the $3 \times 3$ grid, construct an explicit width-$3$ tree
decomposition (the upper bound is achieved by a path of $3$ bags,
each containing one row plus the next). Draw the decomposition
analogously to Figure~\ref{fig:treewidth}.
\end{exercise}

\begin{exercise}
Draw $K_5$ and $K_{3,3}$ side by side. For each, exhibit a
$K_5$-minor (resp.\ $K_{3,3}$-minor) inside the Petersen graph by
choosing four (resp.\ six) connected branch sets and verifying that
the required adjacencies hold.
\end{exercise}

\subsection{Comprehensive Chapter Exercises}

\paragraph{Easy Questions.}

\begin{exercise}
Prove Mantel's theorem by induction on $n$: remove an edge $uv$ from
a triangle-free graph $G$, bound $|E(G - \{u, v\})|$ inductively,
and account for the removed degrees.
\end{exercise}

\begin{exercise}
Show that the chromatic number of the Tur\'an graph $T(n, r)$ is
exactly $r$. Deduce from Tur\'an's theorem that every graph with
more than $(1 - 1/r) n^2/2$ edges has chromatic number $\ge r + 1$.
\end{exercise}

\begin{exercise}
Prove that $R(3, 3) = 6$ by exhibiting a two-coloring of $K_5$ with
no monochromatic triangle and arguing $R(3,3) \le 6$ directly.
\end{exercise}

\paragraph{Medium Questions.}

\begin{exercise}
Using linearity of expectation, show that every tournament on $n$
vertices contains a Hamiltonian path.
\end{exercise}
...
Use the first-moment method to prove that the threshold for
$G(n, p)$ to contain a triangle is $p = 1/n$: show
$\mathbb{E}[\# K_3] = \binom{n}{3} p^3$ and apply Markov's inequality.
\end{exercise}

\begin{exercise}
Prove Kirchhoff's Matrix-Tree theorem for the complete graph $K_n$
and deduce Cayley's formula: $K_n$ has $n^{n-2}$ spanning trees.
\end{exercise}

\paragraph{Hard Questions.}

\begin{exercise}
Show that every graph with treewidth at most $k$ has at most
$k \cdot n$ edges. Deduce that planar graphs, despite allowing up to
$3n - 6$ edges, may still have large treewidth.
\end{exercise}

\begin{exercise}
Prove that the $n \times n$ grid graph has treewidth exactly $n$.
(The upper bound is easy via a sweeping decomposition; the lower
bound requires a connectivity/cops-and-robbers argument.)
\end{exercise}

\begin{exercise}
Using the triangle removal lemma, prove the following: for every
$\eta > 0$ there exists $n_0$ such that any subset of
$\{1, \dots, n\}$ with $n \ge n_0$ and density at least $\eta$ contains
a $3$-term arithmetic progression. (Construct the Ruzsa $3$-AP graph.)
\end{exercise}

%%% NEW CHAPTER 12 FOLLOWS
\input{chapter12_algebraic.tex}
